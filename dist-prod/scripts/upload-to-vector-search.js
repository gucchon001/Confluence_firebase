...
exports.uploadFileToVectorSearch = uploadFileToVectorSearch;
...
async function uploadToVectorSearch(filename, bucketName) {
    try {
        ...
        try {
            ...
            const response = await axios_1.default.patch(`${importUrl}${queryParams}`, requestBody, {
                headers: {
                    'Authorization': `Bearer ${token.token}`,
                    'Content-Type': 'application/json'
                }
...
async function uploadFileToVectorSearch(filename) {
    try {
        // 環境変数の確認
        const bucketName = process.env.VERTEX_AI_STORAGE_BUCKET || '122015916118-vector-search';
        if (!bucketName) {
