...
const embedding_utils_1 = require("../lib/embedding-utils");
        ...
        while (hasMore) {
            ...
            try {
                ...
                if (recordsToEmbed.length > 0) {
                    console.log(`Generating embeddings for ${recordsToEmbed.length} records with dynamic batch sizing...`);
                    try {
                        // 前処理: 大きすぎるテキストを適切なサイズに調整
                        const preprocessedRecords = recordsToEmbed.map(record => ({
                            ...record,
                            content: error_handling_1.ErrorHandler.preprocessLargeText(record.content, 8000)
                        }));
                        // 動的バッチサイズでエンベディングを生成
                        embeddedRecords = await (0, embedding_utils_1.generateEmbeddingsWithDynamicBatch)(preprocessedRecords);
                        ...
                        // 重大なエラーが発生した場合はログに記録
                        await error_handling_1.ErrorHandler.logError('embedding_batch_generation', embeddingError, {
                            batchCount,
                            recordCount: recordsToEmbed.length,
                            firstRecordId: recordsToEmbed[0]?.id
                        });
                        // エラー時は空の埋め込みで処理を継続
                ...
                hasMore = pages.length === batchSize; // 本番用に正常なループ条件に戻す
            }
            catch (batchError) {
...
async function saveMetadataToFirestore(records) {
    ...
    for (const record of records) {
        try {
            if (record && record.id) {
                // 冪等性を確保して処理
                await error_handling_1.ErrorHandler.processWithIdempotency('save_metadata', `${record.id}-${batchKey}`, async () => {
            ...
            await error_handling_1.ErrorHandler.logError('firestore_save', error, {
                recordId: record.id,
                recordType: 'chunk'
            });
    ...
    if (errorCount > 0) {
        // エラーログに詳細を記録
        await error_handling_1.ErrorHandler.logError('firestore_batch_save', new Error(`${errorCount} records failed to save to Firestore.`), {
