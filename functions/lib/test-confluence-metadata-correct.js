"use strict";
/**
 * 正しいConfluence API設定を使用したメタデータ確認テスト
 */
...
/**
 * Confluenceからページの一覧を取得する
 *
 * @param limit 取得するページ数
 * @returns ページの一覧
 */
async function getConfluencePages(limit = 20) {
    try {
...
async function getConfluencePageDetails(pageId) {
    try {
        ...
        const response = await axios_1.default.get(endpoint, {
            params: {
                expand: 'body.storage,metadata.labels,version,space,ancestors,children.page'
            },
            auth: {
                username,
                password: apiToken
 ...
 * @returns ラベルの一覧
 */
async function getPageLabels(pageId) {
    try {
        console.log(`[getPageLabels] Fetching labels for page ${pageId}`);
        // 正しい環境変数を直接指定
        const baseUrl = 'https://giginc.atlassian.net';
        const username = 'kanri@jukust.jp';
        const apiToken = 'API_TOKEN_PLACEHOLDER'; // 環境変数から取得するか、安全な方法で提供する必要があります
        ...
        console.log(`[getPageLabels] Using endpoint: ${endpoint}`);
        // APIリクエストを送信
        const response = await axios_1.default.get(endpoint, {
            auth: {
                username,
        ...
        // レスポンスを処理
        if (response.data && response.data.results) {
            console.log(`[getPageLabels] Found ${response.data.results.length} labels for page ${pageId}`);
            return response.data.results;
        }
        else {
            console.log(`[getPageLabels] No labels found for page ${pageId}`);
            return [];
        }
    }
    catch (error) {
        console.error(`[getPageLabels] Error fetching labels for page ${pageId}:`, error);
        // エラーの詳細情報を出力
        if (error.response) {
            console.error('[getPageLabels] API response error:', {
                status: error.response.status,
                statusText: error.response.statusText,
                data: error.response.data
            });
            ...
            console.error('[getPageLabels] No response received:', error.request);
        }
        else {
            console.error('[getPageLabels] Request error:', error.message);
        }
        return [];
    }
}
...
function analyzePageMetadata(pages) {
    ...
    // ラベルの確認
    const hasLabels = pages.some(page => {
        var _a, _b;
        return ((_b = (_a = page.metadata) === null || _a === void 0 ? void 0 : _a.labels) === null || _b === void 0 ? void 0 : _b.results) &&
            page.metadata.labels.results.length > 0;
    });
...
function analyzePageDetailMetadata(pageDetails) {
    ...
    if (pageDetails.body && pageDetails.body.storage && pageDetails.body.storage.value) {
        const content = pageDetails.body.storage.value;
        console.log('\n[analyzePageDetailMetadata] Content preview (first 200 chars):');
        console.log(content.substring(0, 200) + (content.length > 200 ? '...' : ''));
    }
    else {
        console.log('\n[analyzePageDetailMetadata] No content found');
    }
}
/**
 * メイン関数
...
async function main() {
    try {
        console.log('===== Confluence APIメタデータ確認テスト開始（正しい設定） =====');
        // Confluence APIからページの一覧を取得
        console.log('\n1. Confluence APIからページの一覧を取得');
        const pages = await getConfluencePages(20);
        // ページのメタデータを分析
        console.log('\n2. ページのメタデータを分析');
        ...
        if (pages.length > 0) {
            ...
            // ラベルを取得
            console.log('\n4. 最初のページのラベルを取得');
            const labels = await getPageLabels(pages[0].id);
            console.log(`[main] Labels for page ${pages[0].id}:`, labels);
        }
        console.log('\n===== Confluence APIメタデータ確認テスト完了 =====');
    }
    catch (error) {
...
// スクリプト実行
main();
//# sourceMappingURL=test-confluence-metadata-correct.js.map