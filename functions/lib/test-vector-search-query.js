    ...
    try {
        ...
        if (response.data.nearestNeighbors &&
            ...
            neighbors.forEach((neighbor, index) => {
                ...
                // メタデータがある場合は表示
                if (neighbor.datapoint.metadata) {
                    console.log('- Metadata:', JSON.stringify(neighbor.datapoint.metadata, null, 2));
                }
                // restrictsがある場合は表示
                if (neighbor.datapoint.restricts) {
                    console.log('- Restricts:', JSON.stringify(neighbor.datapoint.restricts, null, 2));
                }
                // crowding_tagがある場合は表示
                if (neighbor.datapoint.crowding_tag) {
                    console.log(`- Crowding Tag: ${neighbor.datapoint.crowding_tag}`);
                }
            });
        }
        else {
...
async function main() {
    try {
        console.log('===== Vector Search API クエリテスト開始 =====');
        // 環境変数の表示
        console.log('環境変数:');
...
// スクリプト実行
main();
//# sourceMappingURL=test-vector-search-query.js.map