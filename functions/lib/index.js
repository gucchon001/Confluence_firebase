...
exports.scheduledSyncConfluenceData = exports.syncConfluenceData = void 0;
...
const v1_1 = require("@google-cloud/aiplatform/build/src/v1");
// Firebase初期化
admin.initializeApp();
// 環境変数の確認
...
async function getAllSpaceContent(spaceKey, type = 'page', limit = 100) {
    var _a, _b, _c;
    // Firebase config から直接環境変数を取得
    const config = functions.config();
    const baseUrl = ((_a = config.confluence) === null || _a === void 0 ? void 0 : _a.base_url) || process.env.CONFLUENCE_BASE_URL;
    if (!baseUrl) {
        throw new Error('環境変数 CONFLUENCE_BASE_URL が設定されていません。');
    }
    const email = ((_b = config.confluence) === null || _b === void 0 ? void 0 : _b.user_email) || process.env.CONFLUENCE_USER_EMAIL;
    if (!email) {
        throw new Error('環境変数 CONFLUENCE_USER_EMAIL が設定されていません。');
    }
    const apiToken = ((_c = config.confluence) === null || _c === void 0 ? void 0 : _c.api_token) || process.env.CONFLUENCE_API_TOKEN;
    if (!apiToken) {
    ...
    while (hasMore) {
        try {
            const response = await axios_1.default.get(`${baseUrl}/wiki/rest/api/content`, {
                params: {
                    spaceKey,
                    type,
...
async function uploadToVectorSearch(records, projectId, location, indexId) {
    var _a, _b;
    if (!indexId) {
    ...
    try {
        ...
        const config = functions.config();
        const bucketName = ((_a = config.vertexai) === null || _a === void 0 ? void 0 : _a.storage_bucket) ||
        ...
        const indexServiceClient = new v1_1.IndexServiceClient({
        ...
        if (err.response) {
            ...
            if ((_b = err.response.data) === null || _b === void 0 ? void 0 : _b.error) {
...
async function syncConfluenceDataImpl() {
    ...
    // Confluence 設定
    const spaceKey = ((_a = config.confluence) === null || _a === void 0 ? void 0 : _a.space_key) || process.env.CONFLUENCE_SPACE_KEY;
    if (!spaceKey) {
        throw new Error('環境変数 CONFLUENCE_SPACE_KEY が設定されていません。');
    }
    // Vertex AI 設定
    const projectId = ((_b = config.vertexai) === null || _b === void 0 ? void 0 : _b.project_id) || process.env.VERTEX_AI_PROJECT_ID;
    if (!projectId) {
        throw new Error('環境変数 VERTEX_AI_PROJECT_ID が設定されていません。');
    }
    const location = ((_c = config.vertexai) === null || _c === void 0 ? void 0 : _c.location) || process.env.VERTEX_AI_LOCATION;
    if (!location) {
        throw new Error('環境変数 VERTEX_AI_LOCATION が設定されていません。');
    }
    // Vector Search インデックスID
    const indexId = ((_d = config.vertexai) === null || _d === void 0 ? void 0 : _d.index_id) || process.env.VERTEX_AI_INDEX_ID || '';
    ...
    for (const page of targetPages) {
        ...
        const html = ((_f = (_e = page.body) === null || _e === void 0 ? void 0 : _e.storage) === null || _f === void 0 ? void 0 : _f.value) || '';
        const url = `${process.env.CONFLUENCE_BASE_URL}/wiki/spaces/${spaceKey}/pages/${pageId}`;
        const labels = ((_j = (_h = (_g = page.metadata) === null || _g === void 0 ? void 0 : _g.labels) === null || _h === void 0 ? void 0 : _h.results) === null || _j === void 0 ? void 0 : _j.map((label) => label.name)) || [];
        const lastUpdated = ((_k = page.version) === null || _k === void 0 ? void 0 : _k.when) || new Date().toISOString();
        // HTMLからテキストを抽出
...
exports.syncConfluenceData = functions
    ...
    try {
        ...
        res.status(500).json({
            ...
            error: ((_a = error.response) === null || _a === void 0 ? void 0 : _a.data) ? JSON.stringify(error.response.data) : 'No detailed error data'
        });
    }
});
// Pub/Sub関数: スケジュール実行用
exports.scheduledSyncConfluenceData = functions
    .region('asia-northeast1') // 東京リージョン
    .runWith({
    timeoutSeconds: 540, // 9分（最大10分）
    memory: '1GB'
