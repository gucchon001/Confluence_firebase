 ...
 * @param gcsPathOrFilename GCSの完全なパスまたはファイル名
 */
async function uploadToVectorSearch(gcsPathOrFilename: string, bucketName: string): Promise<void> {
  try {
    // パスが gs:// で始まるかチェック
    const isFullPath = gcsPathOrFilename.startsWith('gs://');
    const full = isFullPath ? gcsPathOrFilename : `gs://${bucketName}/${gcsPathOrFilename}`;

    // ディレクトリ優先: ファイルが指定された場合は親ディレクトリに正規化
    // 末尾が '/' で終わらない、かつ拡張子を含む場合はファイルとみなし、親のディレクトリ URI を使用
    let contentsDeltaUri = full;
    if (!full.endsWith('/')) {
      const last = full.split('/').pop() || '';
      if (last.includes('.')) {
        contentsDeltaUri = full.slice(0, full.lastIndexOf('/') + 1);
      }
    }
    
    console.log(`Starting batch update with URI: ${contentsDeltaUri}`);
    ...
    try {
      ...
      if (operationName) {
        ...
        await saveSyncLog('vector_search_import_started', {
          ...
          filename: gcsPathOrFilename, // ファイル名を追加
          gcsPath: contentsDeltaUri, // フルパスを追加
          operationName,
...
async function main() {
  try {
    ...
    if (args.length === 0) {
      console.error('Please provide a filename or a directory path as an argument');
      console.log('Usage: npm run upload:vector-search -- <filename_or_directory_path>');
      process.exit(1);
    }
    
    const gcsPathOrFilename = args[0];
    console.log(`Starting Vector Search import for file/directory: ${gcsPathOrFilename}`);
    
    // Vector Searchにアップロード
    await uploadToVectorSearch(gcsPathOrFilename, bucketName);
 ...
 * @param gcsPathOrFilename GCSの完全なパスまたはファイル名
 */
export async function uploadFileToVectorSearch(gcsPathOrFilename: string): Promise<boolean> {
  try {
    // 環境変数の確認
    ...
    console.error(`Error uploading file or directory ${gcsPathOrFilename} to Vector Search: ${error.message}`);
    return false;
  }
}
