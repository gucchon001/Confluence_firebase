...
describe.skip('Ask Question Action', () => {
  beforeEach(() => {
    ...
    (streamingSummarizeConfluenceDocsBackend as any).mockImplementation(async ({ question, context }) => {
      if (context.length === 0) {
        return {
          answer: '関連する仕様書が見つかりませんでした。',
          references: []
        };
      }
      
      return {
        answer: `「${question}」に対する回答です。${context[0].title}に基づいた情報です。`,
        references: context.map(doc => ({ title: doc.title, url: doc.url }))
      };
    });
  });
  ...
  it('should process a question and return a summary with sources', async () => {
    ...
    expect(result.answer).toContain('ログイン認証の仕組み');
  ...
  it('should return a message when no relevant docs are found', async () => {
    ...
    expect(result.answer).toContain('関連する仕様書が見つかりませんでした');
