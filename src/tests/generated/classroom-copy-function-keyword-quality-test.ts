/**
 * classroom-copy-function キーワード抽出品質テスト
 * 自動生成されたテストケース
 */

async function testClassroomCopyFunctionKeywordExtraction() {
  console.log('🚀 classroom-copy-function キーワード抽出品質テスト開始');
  console.log('=' .repeat(60));

  const query = '教室コピー機能の詳細を教えて';
  ...
  try {
    ...
    const irrelevantKeywords = result.keywords.filter(actual => 
      ...
      !isClassroomCopyFunctionRelated(actual)
    );
  ...
  console.log('');
  console.log('=' .repeat(60));
  console.log('✅ classroom-copy-function キーワード抽出品質テスト完了');
}
...
// テスト実行
testClassroomCopyFunctionKeywordExtraction();
