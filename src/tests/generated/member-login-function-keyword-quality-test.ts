/**
 * member-login-function キーワード抽出品質テスト
 * 自動生成されたテストケース
 */

async function testMemberLoginFunctionKeywordExtraction() {
  console.log('🚀 member-login-function キーワード抽出品質テスト開始');
  ...
  try {
    ...
    const irrelevantKeywords = result.keywords.filter(actual => 
      ...
      !isMemberLoginFunctionRelated(actual)
    );
  ...
  console.log('');
  console.log('=' .repeat(60));
  console.log('✅ member-login-function キーワード抽出品質テスト完了');
}
...
// テスト実行
testMemberLoginFunctionKeywordExtraction();
