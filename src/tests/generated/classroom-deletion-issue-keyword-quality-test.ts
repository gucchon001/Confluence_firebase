/**
 * classroom-deletion-issue キーワード抽出品質テスト
 * 自動生成されたテストケース
 */

async function testClassroomDeletionIssueKeywordExtraction() {
  console.log('🚀 classroom-deletion-issue キーワード抽出品質テスト開始');
  ...
  try {
    ...
    const irrelevantKeywords = result.keywords.filter(actual => 
      ...
      !isClassroomDeletionIssueRelated(actual)
    );
  ...
  console.log('');
  console.log('=' .repeat(60));
  console.log('✅ classroom-deletion-issue キーワード抽出品質テスト完了');
}
...
// テスト実行
testClassroomDeletionIssueKeywordExtraction();
