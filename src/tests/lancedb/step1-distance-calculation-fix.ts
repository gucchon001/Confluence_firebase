 ...
 * ステップ1: 距離計算方法の修正
 * 
 * ユークリッド距離からコサイン距離に変更し、距離の分布を改善する
 * 1. LanceDBの設定を確認
 * 2. コサイン距離への変更を実装
 * 3. 距離分布の分析と検証
 * 4. テストケースでの品質評価
...
async function analyzeDistanceDistribution(query: string): Promise<void> {
  ...
  try {
    ...
    // ユークリッド距離での検索
    const euclideanResults = await tbl.search(vector).limit(100).toArray();
    const euclideanDistances = euclideanResults.map(r => r._distance || 0);
    
    // コサイン距離での検索
    const cosineResults = await tbl.search(vector)
      .metricType('cosine')
      .limit(100)
      .toArray();
    const cosineDistances = cosineResults.map(r => r._distance || 0);
