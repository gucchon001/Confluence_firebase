/**
 * Confluence特定ページをLanceDBに保存するスクリプト（最小スキーマ版）
 * 
 * 使用方法:
 * npx tsx src/scripts/confluence-fetch-one-to-lancedb.ts <pageId> <tableName> <contentLimit>
 * 例: npx tsx src/scripts/confluence-fetch-one-to-lancedb.ts 740163745 confluence_minimal 800
 */
...
async function main() {
  ...
  const tableName = process.argv[3] || 'confluence_minimal';
  const contentLimit = parseInt(process.argv[4] || '800', 10);

  if (!pageId) {
    console.error('使用方法: npx tsx src/scripts/confluence-fetch-one-to-lancedb.ts <pageId> <tableName> <contentLimit>');
    process.exit(1);
  }

  console.log(`=== Confluence特定ページをLanceDBに保存（最小スキーマ版） ===`);
  ...
  try {
    ...
    try {
      ...
      if (results.length > 0) {
        ...
        console.log(`  ベクトル長: ${record.vector.length}`);
        console.log(`  ベクトル先頭5要素: [${record.vector.slice(0, 5).map(v => v.toFixed(6)).join(', ')}]`);
