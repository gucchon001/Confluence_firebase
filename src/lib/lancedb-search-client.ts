...
export async function searchLanceDB(params: LanceDBSearchParams): Promise<LanceDBSearchResult[]> {
  ...
  try {
    ...
    // Jira対応: tableNameに応じて処理を分岐
    const isJiraTable = tableName === 'jira_issues';
    
    // ★★★ デバッグ: tableNameとisJiraTableの値を確認 ★★★
    // NODE_ENVが設定されていない場合もデバッグログを有効化（開発・テストのため）
    const DEBUG_TITLE_SEARCH = (process.env.NODE_ENV === 'development' || process.env.NODE_ENV === undefined) && (process.env.DEBUG_SEARCH === 'true' || process.env.DEBUG_TITLE_SEARCH === 'true');
    if (DEBUG_TITLE_SEARCH) {
      console.log(`[Title Rescue Search] tableName: ${tableName}, isJiraTable: ${isJiraTable}`);
    }
    
    try {
      // Jiraテーブルの場合: Issue Keyパターン（CTJ-XXXX）を先に検出（タイトル候補生成より優先）
      let detectedIssueKeys: string[] = [];
      if (isJiraTable) {
        if (DEBUG_TITLE_SEARCH) {
          console.log(`[Title Rescue Search] Jira: Starting Issue Key detection for query: ${params.query}`);
        }
        const issueKeyPattern = /CTJ-\d+/i;
        const issueKeyMatches = params.query.match(issueKeyPattern);
        if (DEBUG_TITLE_SEARCH) {
          console.log(`[Title Rescue Search] Jira: Issue Key pattern matches: ${issueKeyMatches ? issueKeyMatches.join(', ') : 'none'}`);
        }
        if (issueKeyMatches && issueKeyMatches.length > 0) {
          detectedIssueKeys = [...new Set(issueKeyMatches.map(m => m.toUpperCase()))];
          if (DEBUG_TITLE_SEARCH) {
            console.log(`[Title Rescue Search] Jira: Detected Issue Keys in query: ${detectedIssueKeys.join(', ')}`);
          }
        }
      } else {
        if (DEBUG_TITLE_SEARCH) {
          console.log(`[Title Rescue Search] Not a Jira table, skipping Issue Key detection`);
        }
      }
      ...
      // ★★★ 改善: 重要なキーワード（indeedなど）を明示的にタイトル候補に追加 ★★★
      // クエリ全体とキーワードから重要なキーワードを抽出
      const queryLower = params.query.toLowerCase();
      const importantKeywords: string[] = [];
      
      // 重要なキーワードのパターン（サービス名、機能名など）
      const importantPatterns = ['indeed', 'feedforce', 'リクルート', 'マイナビ', 'doda', 'en-japan'];
      ...
      for (const kw of finalKeywords) {
        for (const pattern of importantPatterns) {
          if (kw.toLowerCase().includes(pattern.toLowerCase()) && !importantKeywords.includes(pattern)) {
            importantKeywords.push(pattern);
          }
        }
      }
      ...
      if (importantKeywords.length > 0) {
        titles = [...importantKeywords, ...titles];
        if (DEBUG_TITLE_SEARCH) {
          console.log(`[Title Rescue Search] Added important keywords as title candidates: ${importantKeywords.join(', ')}`);
        }
      }
      
      // Issue Keyを優先的に検索候補の先頭に追加
      if (detectedIssueKeys.length > 0) {
        titles = [...detectedIssueKeys, ...titles];
        if (DEBUG_TITLE_SEARCH) {
          console.log(`[Title Rescue Search] Jira: Title candidates after Issue Key detection: ${titles.slice(0, 5).join(', ')}`);
        }
      }
      
      // ★★★ ステップ2最適化: タイトル候補数を制限（10個 → 5個に削減してパフォーマンス改善） ★★★
      // タイトル候補が多すぎると、順次実行で78秒かかる問題があった
      // 品質への影響: 低（タイトル救済検索は補助的な機能のため）
      // Jiraテーブルの場合: Issue Keyを優先するため、先頭のIssue Keyは必ず含める
      // ★★★ 改善: 重要なキーワード（indeedなど）を優先的に保持 ★★★
      if (isJiraTable && titles.length > 5) {
        const issueKeys = titles.filter(t => /^CTJ-\d+$/i.test(t));
        const importantKws = titles.filter(t => {
          const importantPatterns = ['indeed', 'feedforce', 'リクルート', 'マイナビ', 'doda', 'en-japan'];
          return importantPatterns.some(pattern => t.toLowerCase().includes(pattern.toLowerCase()));
        });
        const nonIssueKeys = titles.filter(t => 
          !/^CTJ-\d+$/i.test(t) && 
          !importantKws.includes(t)
        );
        // Issue Key → 重要なキーワード → その他の候補の順で優先（最大5個）
        titles = [...issueKeys, ...importantKws, ...nonIssueKeys].slice(0, 5);
      } else if (titles.length > 5) {
        // Confluenceテーブルの場合も重要なキーワードを優先
        const importantKws = titles.filter(t => {
          const importantPatterns = ['indeed', 'feedforce', 'リクルート', 'マイナビ', 'doda', 'en-japan'];
          return importantPatterns.some(pattern => t.toLowerCase().includes(pattern.toLowerCase()));
        });
        const nonImportantKws = titles.filter(t => !importantKws.includes(t));
        // 重要なキーワード → その他の候補の順で優先（最大5個）
        titles = [...importantKws, ...nonImportantKws].slice(0, 5);
      }
      ...
      if (titles.length > 0) {
        ...
        const titleSearchPromises = titles.map(async (t) => {
          try {
            // キャッシュキーを生成（Jiraテーブルの場合はテーブル名を含める）
            const cacheKey = `title-search:${isJiraTable ? 'jira:' : ''}${t.toLowerCase().trim()}`;
            ...
            // Jiraテーブルの場合とConfluenceテーブルの場合で処理を分岐
            if (isJiraTable) {
              // Jiraテーブル用のタイトル救済検索
              
              // Issue Keyパターン（CTJ-XXXX）の場合は完全一致検索
              const issueKeyPattern = /^CTJ-\d+$/i;
              if (issueKeyPattern.test(t.trim())) {
                try {
                  const issueKey = t.trim().toUpperCase();
                  const DEBUG_SEARCH_TITLE = process.env.NODE_ENV === 'development' && process.env.DEBUG_SEARCH === 'true';
                  if (DEBUG_SEARCH_TITLE) {
                    console.log(`[Title Rescue Search] Jira: Searching by Issue Key: ${issueKey}`);
                  }
                  const issueRows = await tbl.query().where(`\`id\` = '${issueKey.replace(/'/g, "''")}'`).limit(1).toArray();
                  if (DEBUG_SEARCH_TITLE) {
                    console.log(`[Title Rescue Search] Jira: Found ${issueRows.length} results for Issue Key: ${issueKey}`);
                  }
                  searchResults = issueRows;
                  // Issue Key検索の結果には特別なマーカーを付与
                  if (issueRows.length > 0) {
                    searchResults = issueRows.map(row => ({ ...row, _sourceType: 'title-exact', _issueKeyExact: true }));
                  }
                } catch (e) {
                  console.warn('[searchLanceDB] Failed to search by Issue Key:', t, e);
                  searchResults = [];
                }
              } else {
                // タイトル（summary）でのLIKE検索またはLunr検索
                ...
                if (jiraLunrReady && lunrSearchClient.isReady('jira_issues')) {
                  ...
                  if (lunrResults.length > 0) {
                    ...
                    if (uniqueIssueKeys.length > 0) {
                      try {
                        // すべてのissue_keyを一度に取得（IN句を使用）
                        const issueKeyConditions = uniqueIssueKeys.map(key => `\`id\` = '${String(key).replace(/'/g, "''")}'`).join(' OR ');
                        const allIssueRows = await tbl.query().where(`(${issueKeyConditions})`).limit(200).toArray();
                        
                        // Lunr結果のスコア順にソートしてマッチング
                        const issueRowsMap = new Map<string, any[]>();
                        for (const row of allIssueRows) {
                          const issueKey = row.id || row.issue_key;
                          if (issueKey && !issueRowsMap.has(issueKey)) {
                            issueRowsMap.set(issueKey, []);
                          }
                          if (issueKey) {
                            issueRowsMap.get(issueKey)!.push(row);
                          }
                        }
                        ...
                        searchResults = lunrResults
                          .map(result => {
                            const issueKey = result.id || result.issue_key;
                            return issueKey ? (issueRowsMap.get(issueKey) || []) : [];
                          })
                          .flat()
                          .slice(0, 15); // 最大15件に制限
                      } catch (e) {
                        console.warn('[searchLanceDB] Failed to batch fetch Jira records:', e);
                        // フォールバック: 個別クエリ
                        const lanceDbResults = await Promise.all(lunrResults.map(async (result) => {
                          try {
                            ...
                            const issueRows = await tbl.query().where(`\`id\` = '${String(issueKey).replace(/'/g, "''")}'`).limit(1).toArray();
                            return issueRows;
                          } catch (err) {
                            console.warn('[searchLanceDB] Failed to fetch Jira record for issue_key:', result.id || result.issue_key, err);
                            return [];
                          }
                        }));
                    ...
                    // Lunr検索が0件の場合、LIKE検索にフォールバック
                    const like = `%${t.replace(/'/g, "''")}%`;
                    const exactRows = await tbl.query().where(`title LIKE '${like}'`).limit(15).toArray();
                    searchResults = exactRows;
                    if (DEBUG_TITLE_SEARCH && exactRows.length > 0) {
                      console.log(`[Title Rescue Search] Lunr検索が0件のため、LIKE検索にフォールバック: "${t}" → ${exactRows.length}件`);
                    }
                  }
                } else {
        ...
        // 結果をマージ（Issue Key完全一致の結果を優先）
        const added: any[] = [];
        const issueKeyExactResults: any[] = [];
        
        // ★★★ デバッグ: ベクトル検索の結果にindeed関連チケットが含まれているか確認 ★★★
        if (DEBUG_TITLE_SEARCH) {
          const vectorIndeedResults = vectorResults.filter(r => {
            const title = (r.title || '').toLowerCase();
            return title.includes('indeed');
          });
          if (vectorIndeedResults.length > 0) {
            console.log(`[Title Rescue Search] ⚠️ ベクトル検索の結果に既にindeed関連チケットが${vectorIndeedResults.length}件含まれています:`, vectorIndeedResults.map(r => `${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (距離: ${r._distance?.toFixed(4) || 'N/A'})`).join(', '));
          }
        }
        ...
        for (const exactRows of titleSearchResults) {
          for (const row of exactRows) {
            // Issue Key完全一致の結果を分離
            if ((row as any)._issueKeyExact === true) {
              // 既存に同一idが無ければ追加
              if (!issueKeyExactResults.some(r => r.id === row.id)) {
                // Issue Key完全一致は最高優先度（距離0.0、RRFスコア1.0）
                issueKeyExactResults.push({ 
                  ...row, 
                  _distance: 0.0, 
                  _sourceType: 'title-exact',
                  _rrfScore: 1.0, // ★★★ 修正: RRFスコアを最高値に設定（最優先でソートされるように）
                  _hybridScore: 0.0, // ハイブリッドスコアも最高優先度
                  _compositeScore: 1.0 // Composite Scoreも最高値
                });
              }
            } else {
              // 既存に同一idが無ければ合流
              const existingIndex = vectorResults.findIndex(r => r.id === row.id);
              if (existingIndex === -1) {
                // ★★★ 改善: タイトル救済検索の結果の距離を、タイトルマッチの品質に応じて調整 ★★★
                // 重要なキーワード（indeedなど）を含むタイトルは距離を小さくする（優先度を上げる）
                const title = (row.title || '').toLowerCase();
                const importantKeywords = ['indeed', 'feedforce', 'リクルート', 'マイナビ', 'doda', 'en-japan'];
                const hasImportantKeyword = importantKeywords.some(kw => title.includes(kw.toLowerCase()));
                
                // 重要なキーワードを含む場合は距離を小さく（0.05）、それ以外は0.2
                const distance = hasImportantKeyword ? 0.05 : 0.2;
                // ★★★ 改善: 重要なキーワードを含むチケットに対して、RRFスコアやComposite Scoreを高く設定 ★★★
                // RRFスコアを0.2に設定（距離0.2のチケットのRRFスコア0.0480-0.0593よりも十分に高い）
                const rrfScore = hasImportantKeyword ? 0.2 : undefined; // 距離0.05のチケットに対して、RRFスコアを0.2に設定
                const compositeScore = hasImportantKeyword ? 0.2 : undefined; // Composite Scoreも0.2に設定
                const hybridScore = hasImportantKeyword ? 0.05 : undefined; // ハイブリッドスコアも距離に合わせて設定
                added.push({ 
                  ...row, 
                  _distance: distance, 
                  _sourceType: 'title-exact',
                  ...(rrfScore !== undefined && { _rrfScore: rrfScore }),
                  ...(compositeScore !== undefined && { _compositeScore: compositeScore }),
                  ...(hybridScore !== undefined && { _hybridScore: hybridScore })
                });
              } else {
                // ★★★ 修正: 既存の結果がベクトル検索の結果で、タイトル救済検索で見つかった場合、_sourceType: 'title-exact'を設定してRRF計算に含める ★★★
                const existing = vectorResults[existingIndex];
                
                // タイトル救済検索で見つかった結果には、必ず_sourceType: 'title-exact'を設定
                // これにより、RRF計算でタイトル順位が正しく計算される
                if (!existing._sourceType || existing._sourceType !== 'title-exact') {
                  // 既存の結果に_sourceType: 'title-exact'を追加（タイトル救済検索で見つかったことを示す）
                  existing._sourceType = 'title-exact';
                  
                  if (DEBUG_TITLE_SEARCH) {
                    console.log(`[Title Rescue Search] ✅ 既存の結果にtitle-exactフラグを追加: ${row.issue_key || row.id}: ${row.title?.substring(0, 50)} (既存の距離: ${existing._distance?.toFixed(4) || 'N/A'})`);
                  }
                }
              }
            }
        ...
        // Issue Key完全一致の結果を先頭に追加（最優先）
        if (issueKeyExactResults.length > 0) {
          if (DEBUG_SEARCH || DEBUG_TITLE_SEARCH) {
            console.log(`[Title Rescue Search] Added ${issueKeyExactResults.length} Issue Key exact match results (highest priority, RRF=1.0)`);
            console.log(`[Title Rescue Search] Issue Key exact match issue_keys: ${issueKeyExactResults.map(r => r.issue_key || r.id).join(', ')}`);
            console.log(`[Title Rescue Search] vectorResults count before: ${vectorResults.length}, after: ${vectorResults.length + issueKeyExactResults.length}`);
          }
          vectorResults = [...issueKeyExactResults, ...vectorResults];
          if (DEBUG_SEARCH || DEBUG_TITLE_SEARCH) {
            console.log(`[Title Rescue Search] vectorResults count after adding: ${vectorResults.length}`);
            // 最初の3件を確認
            for (let i = 0; i < Math.min(3, vectorResults.length); i++) {
              const r = vectorResults[i];
              console.log(`[Title Rescue Search]   ${i+1}. ${r.issue_key || r.id} - Issue Key Exact: ${(r as any)._issueKeyExact}`);
            }
          }
        }
        ...
        if (added.length > 0) {
          if (DEBUG_SEARCH || DEBUG_TITLE_SEARCH) {
            ...
            // indeed関連チケットが含まれているか確認
            const indeedResults = added.filter(r => {
              const title = (r.title || '').toLowerCase();
              return title.includes('indeed');
            });
            if (indeedResults.length > 0) {
              console.log(`[Title Rescue Search] ✅ Indeed関連チケットが${indeedResults.length}件含まれています:`, indeedResults.map(r => `${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (距離: ${r._distance?.toFixed(4) || 'N/A'})`).join(', '));
            }
          }
          vectorResults = vectorResults.concat(added);
        } else if (issueKeyExactResults.length === 0) {
          if (DEBUG_SEARCH || DEBUG_TITLE_SEARCH) {
            console.log(`[Title Rescue Search] No title-exact results added (titles searched: ${titles.length})`);
          }
        }
        
        // ★★★ デバッグ: マージ後のvectorResultsにindeed関連チケットが含まれているか確認 ★★★
        if (DEBUG_TITLE_SEARCH) {
          const mergedIndeedResults = vectorResults.filter(r => {
            const title = (r.title || '').toLowerCase();
            return title.includes('indeed');
          });
          if (mergedIndeedResults.length > 0) {
            console.log(`[Title Rescue Search] ✅ マージ後のvectorResultsにindeed関連チケットが${mergedIndeedResults.length}件含まれています:`);
            mergedIndeedResults.forEach((r, i) => {
              console.log(`  ${i + 1}. ${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (距離: ${r._distance?.toFixed(4) || 'N/A'}, ソース: ${(r as any)._sourceType || 'N/A'})`);
            });
          } else {
            console.log(`[Title Rescue Search] ❌ マージ後のvectorResultsにindeed関連チケットが含まれていません`);
          }
        }
      }
      } catch (e) {
    ...
    // ★★★ デバッグ: タイトル救済検索後のIssue Key完全一致の結果を確認
    const DEBUG_SEARCH_HYBRID = process.env.NODE_ENV === 'development' && process.env.DEBUG_SEARCH === 'true';
    if (DEBUG_SEARCH_HYBRID && tableName === 'jira_issues') {
      const issueKeyExactAfterTitleRescue = vectorResults.filter(r => (r as any)._issueKeyExact === true);
      console.log(`[Hybrid Search] After title rescue search: ${vectorResults.length} results, Issue Key exact: ${issueKeyExactAfterTitleRescue.length}`);
      if (issueKeyExactAfterTitleRescue.length > 0) {
        console.log(`[Hybrid Search] Issue Key exact match issue_keys after title rescue: ${issueKeyExactAfterTitleRescue.map(r => r.issue_key || r.id).join(', ')}`);
      }
    }
    ...
    try {
      ...
      for (let i = 0; i < vectorResults.length; i++) {
        ...
        // Issue Key完全一致のフラグを確実に保持（スプレッド構文でコピーされたものを明示的に設定）
        if ((originalResult as any)._issueKeyExact === true) {
          (resultWithScore as any)._issueKeyExact = true;
        }
      ...
      // ★★★ デバッグ: ハイブリッドスコア追加後のIssue Key完全一致の結果を確認
      if (DEBUG_SEARCH_HYBRID && tableName === 'jira_issues') {
        const issueKeyExactAfterHybridScore = resultsWithHybridScore.filter(r => (r as any)._issueKeyExact === true);
        console.log(`[Hybrid Search] After hybrid score processing: ${resultsWithHybridScore.length} results, Issue Key exact: ${issueKeyExactAfterHybridScore.length}`);
        if (issueKeyExactAfterHybridScore.length > 0) {
          console.log(`[Hybrid Search] Issue Key exact match issue_keys after hybrid score: ${issueKeyExactAfterHybridScore.map(r => r.issue_key || r.id).join(', ')}`);
        } else {
          const beforeCount = vectorResults.filter(r => (r as any)._issueKeyExact === true).length;
          if (beforeCount > 0) {
            console.log(`[Hybrid Search] ⚠️ WARNING: Issue Key exact match results were lost during hybrid score processing! (before: ${beforeCount}, after: 0)`);
          }
        }
      }
      
      // 除外件数のログ（ラベルフィルタリングは事前に適用済み）
      if (excludedCount > 0 && DEBUG_SEARCH_HYBRID) {
        ...
        console.log(`[searchLanceDB] Merging ${bm25Results.length} BM25 results into candidates`);
          ...
          for (const row of bm25Results) {
            ...
            if (existingIndex === -1) {
              ...
              // ★★★ 修正: Issue Key完全一致のフラグを保持（既存の結果にBM25スコアを追加する場合でも保持）
              if ((existing as any)._issueKeyExact === true) {
                // 既にIssue Key完全一致のフラグが設定されている場合は保持
              }
            }
          }
          if (DEBUG_SEARCH) {
      ...
      // ★★★ デバッグ: BM25結果マージ後のIssue Key完全一致の結果を確認
      if (DEBUG_SEARCH_HYBRID && tableName === 'jira_issues') {
        const issueKeyExactAfterBM25Merge = resultsWithHybridScore.filter(r => (r as any)._issueKeyExact === true);
        console.log(`[Hybrid Search] After BM25 merge: ${resultsWithHybridScore.length} results, Issue Key exact: ${issueKeyExactAfterBM25Merge.length}`);
        if (issueKeyExactAfterBM25Merge.length > 0) {
          console.log(`[Hybrid Search] Issue Key exact match issue_keys after BM25 merge: ${issueKeyExactAfterBM25Merge.map(r => r.issue_key || r.id).join(', ')}`);
        }
      }
      
      // ★★★ ステップ5最適化: RRF計算前に早期絞り込み（全件 → 75件に削減してパフォーマンス改善） ★★★
      // 品質への影響: 中程度（RRF計算前に結果を絞り込むことで、関連性の高いページが除外される可能性）
      // ⚠️ 注意: 100件以下に削減する場合は、必ず品質テストを実施して合格基準を満たすことを確認
      // ✅ 品質テスト結果: 75件で順位4位を維持（ベースライン6位から改善）、50件で順位5位に下降
      // ✅ 推奨: 75件を採用（品質とパフォーマンスのバランスが最適）
      // ★★★ 修正: Issue Key完全一致の結果を必ず含める
      const RRF_CANDIDATE_LIMIT = 75;
      
      // Issue Key完全一致の結果を分離
      const issueKeyExactResults = resultsWithHybridScore.filter(r => (r as any)._issueKeyExact === true);
      const otherResults = resultsWithHybridScore.filter(r => (r as any)._issueKeyExact !== true);
      
      // Issue Key完全一致の結果を先頭に、その後に他の結果を追加（最大75件）
      const topCandidatesForRRF = [...issueKeyExactResults, ...otherResults].slice(0, RRF_CANDIDATE_LIMIT);
      const remainingCandidates = [...issueKeyExactResults, ...otherResults].slice(RRF_CANDIDATE_LIMIT);
      
      // ★★★ デバッグ: RRF処理前の絞り込み後のIssue Key完全一致の結果を確認
      if (DEBUG_SEARCH_HYBRID && tableName === 'jira_issues') {
        const issueKeyExactAfterSlice = topCandidatesForRRF.filter(r => (r as any)._issueKeyExact === true);
        console.log(`[Hybrid Search] After slice for RRF (top ${RRF_CANDIDATE_LIMIT}): ${topCandidatesForRRF.length} results, Issue Key exact: ${issueKeyExactAfterSlice.length}`);
        if (issueKeyExactAfterSlice.length > 0) {
          console.log(`[Hybrid Search] Issue Key exact match issue_keys after slice: ${issueKeyExactAfterSlice.map(r => r.issue_key || r.id).join(', ')}`);
        } else if (issueKeyExactResults.length > 0) {
          console.log(`[Hybrid Search] ⚠️ WARNING: Issue Key exact match results were lost during slice! (before: ${issueKeyExactResults.length}, after: 0)`);
        }
      }
      
      // ★★★ デバッグ: RRF処理前のindeed関連チケットの順位を確認 ★★★
      const DEBUG_INDEED_TRACKING = process.env.NODE_ENV === 'development' && (process.env.DEBUG_SEARCH === 'true' || process.env.DEBUG_TITLE_SEARCH === 'true');
      if (DEBUG_INDEED_TRACKING && tableName === 'jira_issues' && params.query?.toLowerCase().includes('indeed')) {
        const indeedBeforeRRF = topCandidatesForRRF
          ...
          .sort((a, b) => {
            const aScore = (a as any)._rrfScore ?? (a as any)._hybridScore ?? a._distance ?? 1.0;
            const bScore = (b as any)._rrfScore ?? (b as any)._hybridScore ?? b._distance ?? 1.0;
            return bScore - aScore;
          });
        console.log(`[RRF Tracking] RRF処理前: indeed関連チケット ${indeedBeforeRRF.length}件`);
        indeedBeforeRRF.slice(0, 5).forEach((r, i) => {
          console.log(`  ${i + 1}. ${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (距離: ${r._distance?.toFixed(4) || 'N/A'}, RRF: ${((r as any)._rrfScore?.toFixed(4) || 'N/A')}, 元の順位: ${(r as any)._originalIndex + 1})`);
        });
      }
      
      const rrfProcessedResults = await unifiedSearchResultProcessor.processSearchResults(topCandidatesForRRF, {
        vectorWeight: 0.4,
        keywordWeight: 0.4,
        labelWeight: 0.2,
      ...
      // ★★★ デバッグ: RRF処理後のindeed関連チケットの順位を確認 ★★★
      if (DEBUG_INDEED_TRACKING && tableName === 'jira_issues' && params.query?.toLowerCase().includes('indeed')) {
        const indeedAfterRRF = rrfProcessedResults
          .map((r, idx) => ({ ...r, _originalIndex: idx }))
          .filter(r => {
            const title = (r.title || '').toLowerCase();
            return title.includes('indeed');
        ...
        console.log(`[RRF Tracking] RRF処理後: indeed関連チケット ${indeedAfterRRF.length}件`);
        indeedAfterRRF.slice(0, 5).forEach((r, i) => {
          console.log(`  ${i + 1}. ${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (距離: ${r._distance?.toFixed(4) || 'N/A'}, RRF: ${((r as any)._rrfScore?.toFixed(4) || 'N/A')}, 元の順位: ${(r as any)._originalIndex + 1})`);
        });
      }
      
      // 残りの候補には簡易RRFスコアを設定（RRF計算をスキップしてパフォーマンス改善）
      const remainingWithSimpleRRF = remainingCandidates.map(r => ({
        ...r,
        _rrfScore: (r._hybridScore ?? r._distance ?? 1.0) * 0.1 // 簡易スコア（RRF計算をスキップ）
      }));
      
      // RRF処理済み結果と残りの候補をマージ
      resultsWithHybridScore = [...rrfProcessedResults, ...remainingWithSimpleRRF];
      
      // ★★★ デバッグ: 重複除去処理前のindeed関連チケットの順位を確認 ★★★
      if (DEBUG_INDEED_TRACKING && tableName === 'jira_issues' && params.query?.toLowerCase().includes('indeed')) {
        const indeedBeforeDedup = resultsWithHybridScore
          .map((r, idx) => ({ ...r, _originalIndex: idx }))
          .filter(r => {
            const title = (r.title || '').toLowerCase();
            return title.includes('indeed');
        ...
        console.log(`[Dedup Tracking] 重複除去処理前: indeed関連チケット ${indeedBeforeDedup.length}件`);
        indeedBeforeDedup.slice(0, 5).forEach((r, i) => {
          console.log(`  ${i + 1}. ${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (距離: ${r._distance?.toFixed(4) || 'N/A'}, RRF: ${((r as any)._rrfScore?.toFixed(4) || 'N/A')}, 元の順位: ${(r as any)._originalIndex + 1})`);
        });
      }

      // ★★★ ステップ3最適化: 重複除去処理を最適化（RRFスコアで事前ソートして最良スコアを優先的に保持） ★★★
      // 同一pageId/titleの重複を1件に正規化（最良スコアを残す）
      // ★★★ MIGRATION: pageId取得を両方のフィールド名に対応 ★★★
      const { getPageIdFromRecord } = await import('./pageid-migration-helper');
      const dedupMap = new Map<string, any>();
      
      // ★★★ 最適化: RRFスコアで事前ソートして、最良スコアを優先的に保持 ★★★
      // ★★★ 修正: Issue Key完全一致の結果を最優先に配置
      // 重複除去処理の説明:
      // 1. 事前ソート: Issue Key完全一致 > RRFスコア（降順）でソート
      // 2. 重複判定: Jiraテーブルはissue_key、ConfluenceテーブルはpageIdで重複判定
      // 3. 重複時の処理: Issue Key完全一致を最優先、次にスコアが良い方を保持
      const sortedForDedup = [...resultsWithHybridScore].sort((a, b) => {
        ...
        // どちらもIssue Key完全一致、またはどちらも違う場合はRRFスコアでソート
      ...
      for (const r of sortedForDedup) {
        ...
        // 既に存在する場合は、Issue Key完全一致の結果を優先的に保持
        const existing = dedupMap.get(key);
        if (!existing) {
          dedupMap.set(key, r);
        } else {
          // ★★★ 修正: Issue Key完全一致の結果を最優先に保持
          const isIssueKeyExact = (r as any)._issueKeyExact === true;
          const existingIsIssueKeyExact = (existing as any)._issueKeyExact === true;
          
          // Issue Key完全一致の結果を優先的に保持
          if (isIssueKeyExact && !existingIsIssueKeyExact) {
            // 新しい結果がIssue Key完全一致で、既存の結果が違う場合は置き換え
            dedupMap.set(key, r);
          } else if (!isIssueKeyExact && existingIsIssueKeyExact) {
            // 既存の結果がIssue Key完全一致の場合は、既存の結果を保持（何もしない）
          } else {
            // どちらもIssue Key完全一致、またはどちらも違う場合は、スコアが良い方を保持
            const rScore = r._rrfScore ?? r._hybridScore ?? (r._distance ?? 1.0);
            const existingScore = existing._rrfScore ?? existing._hybridScore ?? (existing._distance ?? 1.0);
            // 距離の場合は小さい方が良い、スコアの場合は大きい方が良い
            const rValue = r._distance !== undefined ? -r._distance : rScore;
            const existingValue = existing._distance !== undefined ? -existing._distance : existingScore;
            if (rValue > existingValue) {
              dedupMap.set(key, r);
            }
          }
        }
      ...
      // ★★★ 重要: dedupMapから取得した結果は、Mapの挿入順序が保持されるが、
      //     置き換えが発生した場合、最初に追加されたエントリの位置が保持されるため、
      //     RRFスコア順が崩れる可能性がある。そのため、必ず再ソートする必要がある。
      // ★★★ 修正: 重複除去処理後、必ずRRFスコアで再ソートして順位を維持 ★★★
      const dedupedResultsSorted = dedupedResults.sort((a, b) => {
        // Issue Key完全一致の結果を最優先
        const aIsIssueKeyExact = (a as any)._issueKeyExact === true;
        const bIsIssueKeyExact = (b as any)._issueKeyExact === true;
      ...
      // ★★★ デバッグ: 重複除去処理後のindeed関連チケットの順位を確認（ソート前） ★★★
      if (DEBUG_INDEED_TRACKING && tableName === 'jira_issues' && params.query?.toLowerCase().includes('indeed')) {
        const indeedAfterDedupBeforeSort = dedupedResults
          .map((r, idx) => ({ ...r, _originalIndex: idx }))
          .filter(r => {
            const title = (r.title || '').toLowerCase();
            return title.includes('indeed');
          });
        console.log(`[Dedup Tracking] 重複除去処理後（ソート前）: indeed関連チケット ${indeedAfterDedupBeforeSort.length}件`);
        indeedAfterDedupBeforeSort.forEach((r, i) => {
          console.log(`  ${(r as any)._originalIndex + 1}位. ${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (距離: ${r._distance?.toFixed(4) || 'N/A'}, RRF: ${((r as any)._rrfScore?.toFixed(4) || 'N/A')})`);
        });
        
        // ★★★ デバッグ: ソート前の上位10件のRRFスコアを確認 ★★★
        console.log(`[Dedup Tracking] ソート前の上位10件のRRFスコア:`);
        dedupedResults.slice(0, 10).forEach((r, i) => {
          const rrfScore = (r as any)._rrfScore ?? r._hybridScore ?? r._distance ?? 1.0;
          console.log(`  ${i + 1}位. ${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (RRF: ${rrfScore.toFixed(4)})`);
        });
        
        const indeedAfterDedupAfterSort = dedupedResultsSorted
          .map((r, idx) => ({ ...r, _originalIndex: idx }))
          .filter(r => {
            const title = (r.title || '').toLowerCase();
            return title.includes('indeed');
          });
        console.log(`[Dedup Tracking] 重複除去処理後（ソート後）: indeed関連チケット ${indeedAfterDedupAfterSort.length}件`);
        indeedAfterDedupAfterSort.forEach((r, i) => {
          console.log(`  ${(r as any)._originalIndex + 1}位. ${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (距離: ${r._distance?.toFixed(4) || 'N/A'}, RRF: ${((r as any)._rrfScore?.toFixed(4) || 'N/A')})`);
        });
        
        // ★★★ デバッグ: ソート後の上位10件のRRFスコアを確認 ★★★
        console.log(`[Dedup Tracking] ソート後の上位10件のRRFスコア:`);
        dedupedResultsSorted.slice(0, 10).forEach((r, i) => {
          const rrfScore = (r as any)._rrfScore ?? r._hybridScore ?? r._distance ?? 1.0;
          console.log(`  ${i + 1}位. ${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (RRF: ${rrfScore.toFixed(4)})`);
        });
      }
      ...
      // ★★★ 修正: Issue Key完全一致の結果を最優先に配置
      // ★★★ 改善: 重複除去処理後もRRFスコアでソートして、順位を維持 ★★★
      // 注意: dedupedResultsSortedは既にソート済みだが、タイブレークのために再度ソート
      vectorResults = dedupedResultsSorted.sort((a, b) => {
        // Issue Key完全一致の結果を最優先
        const aIsIssueKeyExact = (a as any)._issueKeyExact === true;
        const bIsIssueKeyExact = (b as any)._issueKeyExact === true;
      ...
      // ★★★ デバッグ: RRFソート後のindeed関連チケットの順位を確認 ★★★
      if (DEBUG_INDEED_TRACKING && tableName === 'jira_issues' && params.query?.toLowerCase().includes('indeed')) {
        const indeedAfterRRFSort = vectorResults
          .map((r, idx) => ({ ...r, _finalIndex: idx }))
          .filter(r => {
            const title = (r.title || '').toLowerCase();
            return title.includes('indeed');
          });
        console.log(`[RRF Sort Tracking] RRFソート後: indeed関連チケット ${indeedAfterRRFSort.length}件`);
        indeedAfterRRFSort.forEach((r, i) => {
          console.log(`  ${(r as any)._finalIndex + 1}位. ${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (距離: ${r._distance?.toFixed(4) || 'N/A'}, RRF: ${((r as any)._rrfScore?.toFixed(4) || 'N/A')})`);
        });
      }
      ...
      try {
        ...
        // ⚡ ログ削減: デバッグ時のみ詳細ログを出力
        const DEBUG_SEARCH_COMPOSITE = process.env.NODE_ENV === 'development' && process.env.DEBUG_SEARCH === 'true';
        
        // Phase 6最適化: RRFスコアでソート（早期絞り込み）
        // ★★★ 修正: Issue Key完全一致の結果を最優先に配置
        // ★★★ デバッグ: RRFソート前のIssue Key完全一致の結果を確認
        if (DEBUG_SEARCH_COMPOSITE && tableName === 'jira_issues') {
          const issueKeyExactBeforeRRFSort = vectorResults.filter(r => (r as any)._issueKeyExact === true);
          console.log(`[RRF Sort] Before RRF sort: ${vectorResults.length} results, Issue Key exact: ${issueKeyExactBeforeRRFSort.length}`);
          if (issueKeyExactBeforeRRFSort.length > 0) {
            console.log(`[RRF Sort] Issue Key exact match issue_keys before sort: ${issueKeyExactBeforeRRFSort.map(r => r.issue_key || r.id).join(', ')}`);
          }
        }
        
        const rrfSorted = vectorResults.sort((a, b) => {
          // Issue Key完全一致の結果を最優先
          const aIsIssueKeyExact = (a as any)._issueKeyExact === true;
          const bIsIssueKeyExact = (b as any)._issueKeyExact === true;
          ...
          // どちらもIssue Key完全一致、またはどちらも違う場合はRRFスコアでソート
          return (b._rrfScore || 0) - (a._rrfScore || 0);
        });
        
        // ★★★ デバッグ: RRFソート後のIssue Key完全一致の結果を確認
        if (DEBUG_SEARCH_COMPOSITE && tableName === 'jira_issues') {
          const issueKeyExactAfterRRFSort = rrfSorted.filter(r => (r as any)._issueKeyExact === true);
          console.log(`[RRF Sort] After RRF sort: ${rrfSorted.length} results, Issue Key exact: ${issueKeyExactAfterRRFSort.length}`);
          if (issueKeyExactAfterRRFSort.length > 0) {
            console.log(`[RRF Sort] Issue Key exact match issue_keys after sort: ${issueKeyExactAfterRRFSort.map(r => r.issue_key || r.id).join(', ')}`);
          } else {
            const beforeCount = vectorResults.filter(r => (r as any)._issueKeyExact === true).length;
            if (beforeCount > 0) {
              console.log(`[RRF Sort] ⚠️ WARNING: Issue Key exact match results were lost during RRF sort! (before: ${beforeCount}, after: 0)`);
            }
          }
        }
        
        // ⚡ ログ削減: デバッグ時のみ出力
        if (DEBUG_SEARCH_COMPOSITE) {
        console.log(`[Phase 6 Optimization] Total candidates: ${rrfSorted.length}`);
        }
        
        // ★★★ ステップ4最適化: 上位50件のみComposite Scoringを実行（75件 → 50件にさらに削減してパフォーマンス改善） ★★★
        // 品質への影響: 中程度（スコアリング対象が減ることで、関連性の高いページが除外される可能性）
        // ★★★ 修正: Issue Key完全一致の結果を必ず含める
        const TOP_N_FOR_COMPOSITE = 50;
        
        // Issue Key完全一致の結果を分離
        const issueKeyExactResults = rrfSorted.filter(r => (r as any)._issueKeyExact === true);
        const otherResults = rrfSorted.filter(r => (r as any)._issueKeyExact !== true);
        
        // Issue Key完全一致の結果を先頭に、その後に他の結果を追加（最大50件）
        const top100 = [...issueKeyExactResults, ...otherResults].slice(0, TOP_N_FOR_COMPOSITE);
        const remaining = [...issueKeyExactResults, ...otherResults].slice(TOP_N_FOR_COMPOSITE);
        
        if (DEBUG_SEARCH_COMPOSITE) {
        console.log(`[Phase 6 Optimization] Applying composite scoring to top ${top100.length} results only`);
        }
        
        // ★★★ デバッグ: Composite Scoring処理前のIssue Key完全一致の結果を確認
        if (DEBUG_SEARCH_COMPOSITE && tableName === 'jira_issues') {
          const issueKeyExactBeforeComposite = top100.filter(r => (r as any)._issueKeyExact === true);
          console.log(`[Composite Scoring] Before scoring: ${top100.length} results, Issue Key exact: ${issueKeyExactBeforeComposite.length}`);
          if (issueKeyExactBeforeComposite.length > 0) {
            console.log(`[Composite Scoring] Issue Key exact match issue_keys before: ${issueKeyExactBeforeComposite.map(r => r.issue_key || r.id).join(', ')}`);
          }
        }
        ...
        // ★★★ デバッグ: Composite Scoring処理後のindeed関連チケットの順位を確認 ★★★
        if (DEBUG_INDEED_TRACKING && tableName === 'jira_issues' && params.query?.toLowerCase().includes('indeed')) {
          const indeedAfterComposite = scored100
            .map((r, idx) => ({ ...r, _finalIndex: idx }))
            .filter(r => {
              const title = (r.title || '').toLowerCase();
              return title.includes('indeed');
            });
          console.log(`[Composite Tracking] Composite Scoring後: indeed関連チケット ${indeedAfterComposite.length}件`);
          indeedAfterComposite.forEach((r, i) => {
            console.log(`  ${(r as any)._finalIndex + 1}位. ${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (距離: ${r._distance?.toFixed(4) || 'N/A'}, RRF: ${((r as any)._rrfScore?.toFixed(4) || 'N/A')}, Composite: ${((r as any)._compositeScore?.toFixed(4) || 'N/A')})`);
          });
        }
        
        // ★★★ デバッグ: Composite Scoring処理後のIssue Key完全一致の結果を確認
        if (DEBUG_SEARCH_COMPOSITE && tableName === 'jira_issues') {
          const issueKeyExactAfterComposite = scored100.filter(r => (r as any)._issueKeyExact === true);
          console.log(`[Composite Scoring] After scoring: ${scored100.length} results, Issue Key exact: ${issueKeyExactAfterComposite.length}`);
          if (issueKeyExactAfterComposite.length > 0) {
            console.log(`[Composite Scoring] Issue Key exact match issue_keys after: ${issueKeyExactAfterComposite.map(r => r.issue_key || r.id).join(', ')}`);
          } else {
            const beforeCount = top100.filter(r => (r as any)._issueKeyExact === true).length;
            if (beforeCount > 0) {
              console.log(`[Composite Scoring] ⚠️ WARNING: Issue Key exact match results were lost during scoring! (before: ${beforeCount}, after: 0)`);
            }
          }
        }
        ...
        // ★★★ デバッグ: 最終ソート後のindeed関連チケットの順位を確認 ★★★
        if (DEBUG_INDEED_TRACKING && tableName === 'jira_issues' && params.query?.toLowerCase().includes('indeed')) {
          const indeedAfterFinalSort = vectorResults
            .map((r, idx) => ({ ...r, _finalIndex: idx }))
            .filter(r => {
              const title = (r.title || '').toLowerCase();
              return title.includes('indeed');
            });
          console.log(`[Final Sort Tracking] 最終ソート後: indeed関連チケット ${indeedAfterFinalSort.length}件`);
          indeedAfterFinalSort.forEach((r, i) => {
            console.log(`  ${(r as any)._finalIndex + 1}位. ${r.issue_key || r.id}: ${r.title?.substring(0, 50)} (距離: ${r._distance?.toFixed(4) || 'N/A'}, RRF: ${((r as any)._rrfScore?.toFixed(4) || 'N/A')}, Composite: ${((r as any)._compositeScore?.toFixed(4) || 'N/A')})`);
          });
        }
    ...
    // ★★★ デバッグ: Composite Scoring後のIssue Key完全一致の結果を確認
    const DEBUG_SEARCH_FINAL = process.env.NODE_ENV === 'development' && process.env.DEBUG_SEARCH === 'true';
    if (DEBUG_SEARCH_FINAL && tableName === 'jira_issues') {
      const issueKeyExactAfterComposite = combinedResults.filter(r => (r as any)._issueKeyExact === true);
      console.log(`[Search Final] After composite scoring: ${combinedResults.length} results, Issue Key exact: ${issueKeyExactAfterComposite.length}`);
      if (issueKeyExactAfterComposite.length > 0) {
        console.log(`[Search Final] Issue Key exact match issue_keys: ${issueKeyExactAfterComposite.map(r => r.issue_key || r.id).join(', ')}`);
      }
    }
    ...
    // ★★★ デバッグ: スライス後のIssue Key完全一致の結果を確認
    if (DEBUG_SEARCH_FINAL && tableName === 'jira_issues') {
      const issueKeyExactAfterSlice = finalResults.filter(r => (r as any)._issueKeyExact === true);
      console.log(`[Search Final] After slice (top ${topK * 3}): ${finalResults.length} results, Issue Key exact: ${issueKeyExactAfterSlice.length}`);
      if (issueKeyExactAfterSlice.length > 0) {
        console.log(`[Search Final] Issue Key exact match issue_keys after slice: ${issueKeyExactAfterSlice.map(r => r.issue_key || r.id).join(', ')}`);
      }
    }
    ...
    const processedResults = await unifiedSearchResultProcessor.processSearchResults(filtered, {
      vectorWeight: 0.4,
      keywordWeight: 0.4,
      labelWeight: 0.2,
  ...
  for (const result of results) {
    ...
    if (!existing) {
      ...
      if (isIssueKeyExact && !existingIsIssueKeyExact) {
        // 新しい結果がIssue Key完全一致で、既存の結果が違う場合は置き換え
        pageMap.set(key, result);
        const DEBUG_DEDUP = process.env.NODE_ENV === 'development' && process.env.DEBUG_SEARCH === 'true';
        if (DEBUG_DEDUP) {
          console.log(`[Deduplicator] Replaced with Issue Key exact match: ${result.issue_key || result.id}`);
        }
        continue;
      }
      if (!isIssueKeyExact && existingIsIssueKeyExact) {
        // 既存の結果がIssue Key完全一致の場合は、既存の結果を保持
        continue;
      }
      
      // どちらもIssue Key完全一致、またはどちらも違う場合は、距離で比較
...
async function executeVectorSearch(
  ...
  try {
        ...
        try {
          ...
          vectorResults = vectorResults.map(result => {
            ...
            if (fullRecord && typeof fullRecord === 'object' && fullRecord !== null) {
              // ベクトル検索の距離情報を保持
              // ★★★ 修正: result.distanceではなくresult._distanceを使用（LanceDBのベクトル検索結果では_distanceに格納される） ★★★
              const distance = result._distance ?? result.distance;
              if (distance === undefined || isNaN(distance)) {
                console.warn(`[Vector Search] ⚠️ 距離が無効です: ${result.id || result.issue_key}, distance=${distance}, _distance=${result._distance}`);
              }
              return {
                ...(fullRecord as Record<string, any>),
                _distance: distance ?? 1.0 // 距離が無効な場合は1.0をデフォルト値として設定
              };
            }
            return result;
          });
    ...
    vectorResults = vectorResults.map(result => {
      ...
      // space_keyを確実に保持（spaceKeyも確認）
      const space_key = result.space_key ?? result.spaceKey ?? undefined;
      ...
      if (matchedKeywords.length > 0) {
        ...
        return { 
          ...
          space_key: space_key, // ★★★ space_keyを確実に保持 ★★★
      ...
      // マッチしない場合もpage_id、space_key、Jira特有のフィールドを保持
      return {
        ...result,
        ...jiraFields, // Jira特有のフィールドを追加
        page_id: page_id, // ★★★ MIGRATION: page_idを確実に保持 ★★★
...
async function executeBM25Search(
  ...
  try {
    ...
    if (!isLunrReady) {
      ...
      // ★★★ 修正: 初期化をバックグラウンドで開始（結果を待たない） ★★★
      // 理由: initializeAsync()が既存のPromiseを返す場合、完了まで待ってしまうため、
      //       バックグラウンドで開始するだけで、完了を待たない
      ...
      while (Date.now() - startTime < timeoutMs) {
        // isReady()をチェック（初期化が完了しているか確認）
        isReadyNow = lunrSearchClient.isReady(tableName);
        if (isReadyNow) {
          console.log(`[BM25 Search] ✅ Lunr became ready for ${tableName} within ${Date.now() - startTime}ms`);
          break; // 準備できたので、BM25検索を続行
        }
      ...
      if (!isReadyNow) {
        ...
        console.warn(`[BM25 Search] ⏳ Lunr initialization timeout for ${tableName} (${timeoutMs}ms), skipping BM25 search`);
        console.warn(`[BM25 Search] ⚠️ BM25検索が無効化されます。ベクトル検索のみで結果を返します。`);
        return [];
      }
    }
    
    // 🔍 デバッグ: Lunrインデックスの状態を確認（デバッグ時のみ）
    if (DEBUG_ENABLED) {
    try {
    ...
    // ⚡ 最適化: キーワード検索を並列実行してパフォーマンス向上（2-5秒 → 0.5-1秒に短縮を目指す）
    const resultMap = new Map<string, { result: any; scores: Map<string, number>; matchedKeywords: string[] }>();
    
    // ⚡ 最適化: トークン化を一括処理（パフォーマンス向上）
    const { tokenizeJapaneseTexts } = await import('./japanese-tokenizer');
    const tokenizedQueries = await tokenizeJapaneseTexts(searchKeywords);
    const keywordToTokenizedMap = new Map<string, string>();
    searchKeywords.forEach((keyword, index) => {
      keywordToTokenizedMap.set(keyword, tokenizedQueries[index]);
      if (DEBUG_ENABLED) {
        console.log(`[BM25 Search] Searching '${keyword}' -> '${tokenizedQueries[index]}'`);
      }
    });
    
    // 並列実行: すべてのキーワード検索を同時に実行（トークン化は完了済み）
    const keywordSearchPromises = searchKeywords.map(async (keyword) => {
      const tokenizedQuery = keywordToTokenizedMap.get(keyword)!;
      ...
      return { keyword, results: keywordResults };
    });
    
    // すべてのキーワード検索が完了するまで待機
    const keywordSearchResults = await Promise.all(keywordSearchPromises);
    
    // 結果を統合
    for (const { keyword, results } of keywordSearchResults) {
      for (const result of results) {
    ...
    // ⚡ 最適化: デバッグログを削減（パフォーマンス向上）
    // DEBUG_ENABLEDは関数の最初で既に定義済み
