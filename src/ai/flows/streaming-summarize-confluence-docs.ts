...
6. **重要**: 簡潔で要点を押さえた回答を心がけてください（冗長な説明を避ける）
7. **重要**: ドキュメントタイトル（【FIX】や【NEW】などのプレフィックス付き）を言及する際は、自然な形で言い換えてください
...
export async function* streamingSummarizeConfluenceDocs(
  ...
  try {
    // コンテキストの準備（パフォーマンス最適化: 文書数と文字数を削減）
    const contextText = context
      .slice(0, 3) // パフォーマンス最適化: 5 → 3件に削減
      .map(
        (doc) => {
          // パフォーマンス最適化: 500文字に削減（800 → 500）
...
${truncatedContent}`;
    ...
    try {
      ...
      const generatePromise = ai.generate({
        ...
        prompt: prompt,
        config: {
          maxOutputTokens: 2048, // パフォーマンス最適化: 4096 → 2048
