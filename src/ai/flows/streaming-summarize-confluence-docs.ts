...
本機能は、サービスサイトに会員としてログインするための機能です。

## 👥 詳細

### 1. ログ出力

会員がログインに成功または失敗した場合、以下の情報がログとして出力されます。

#### 成功時:
- **日時**: ログイン成功時の日時
- **ユーザー番号**: ログインした会員のユーザー番号
- **アクセス元IPアドレス**: ログイン元のIPアドレス
- **ユーザーエージェント**: 使用したブラウザ情報

#### 失敗時:
- **日時**: ログイン失敗時の日時
- **メールアドレスの冒頭3文字**: セキュリティのため一部のみ表示
- **アクセス元IPアドレス**: ログイン元のIPアドレス
- **ユーザーエージェント**: 使用したブラウザ情報

### 2. 権限

- **対象**: サービスサイトにログインできる会員のみが本機能を利用できます。

### 3. 関連帳票

- **関連する帳票**: ログイン履歴やセキュリティ監査に関する帳票
\`\`\`
