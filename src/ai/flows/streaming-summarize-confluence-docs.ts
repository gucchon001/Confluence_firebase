...
- **リスト項目内の改行**: 項目名の後に必ず改行して説明文を記載。複数の項目がある場合は、各項目を独立した行に配置
...
- **ログイン失敗回数**: 
  パスワードを10回以上間違えてログインに失敗すると、アカウントがロックされます。

- **ロック解除条件**: 
  アカウントロックから30分経過すると自動的に解除されます。また、パスワード再設定機能を利用してパスワードをリセットした場合もロック状態が解除されます。

- **エラーメッセージ**: 
  攻撃者にヒントを与えないため、アカウントがロックされた旨のエラーメッセージは画面に表示されます。
...
- **箇条書きの記法**: \`- **項目**:\` の後に必ず改行して説明文を記載
- **箇条書きの間隔**: 各リスト項目の間に1行の空行を入れる
- **段落**: 各段落の前後に必ず2行の空行を入れる
- **見出し**: 各見出しの前後に必ず2行の空行を入れる
