...
## 出力形式（必須）
**重要**: 以下のマークダウン形式を厳格に守ってください

- **メインタイトル**: \`## 🔑 機能名の仕様概要\`
- **セクション見出し**: \`## 👥 セクション名\`、\`## 🔗 関連機能\`、\`## ⚙️ 管理機能\`
- **サブセクション**: \`### 1. 項目名\`、\`### 2. 項目名\`
- **箇条書き**: \`- **項目**: 説明\`
- **見出しの前後**: 必ず空行2行を入れる
- **番号付きリスト**: \`1. 項目\`、\`2. 項目\`
...
ログイン認証に関する詳細な仕様を以下に記載します。

## 👥 エラー処理

ログイン認証におけるエラー処理は以下の通りです。

### 1. アカウントが存在しない場合

- **エラーメッセージ**: 攻撃者にはアカウントの有無がわからないようにするため、メールアドレスかパスワードに何らかの誤りがある旨のみのエラーメッセージを表示します。

### 2. アカウントがロックされた場合

- **エラーメッセージ**: 攻撃者にはアカウントがロックされたことがわからないようにするため、アカウントがロックされた旨のエラーメッセージは画面に表示しません。

## 🔗 関連機能

### 1. ログ記録

ログイン認証の各イベントにおいて、以下のフォーマットでログを出力します。
\`\`\`

## 回答
**重要**: 必ずマークダウン形式で回答してください。見出しには必ず##や###を使用し、箇条書きには-を使用してください。

上記のマークダウン形式の例を参考に、以下の構造で回答してください：
1. ## 🔑 機能名の仕様概要 で開始
2. ## 👥 セクション名 で各セクションを区切る
3. ### 1. 項目名 でサブセクションを作成
4. - **項目**: 説明 で箇条書きを使用
...
export async function* streamingSummarizeConfluenceDocs(
  ...
  try {
    // コンテキストの準備（品質重視: 適度な長さを維持）
    const contextText = context
      .slice(0, 3) // 文書数を3件に設定（品質重視）
      .map(
        (doc) => {
          // 各文書の内容を600文字に設定（品質重視）
...
${truncatedContent}`;
    ...
    try {
      ...
      const generatePromise = ai.generate({
        ...
        prompt: prompt,
        config: {
          maxOutputTokens: 3072, // 品質重視: 出力トークンを適度に設定
          temperature: 0.2, // 品質重視: 温度を適度に設定
          topP: 0.85, // 品質重視: topPを適度に設定
          topK: 40, // 品質重視: topKを適度に設定
        }
      });
      
      // 品質重視: 45秒のタイムアウトを設定（適度な余裕を持たせる）
