...
本機能は、登録済みの求人情報を1件閲覧するための機能です。

## 👥 表示項目とロジック

### 1. 教室名

- **表示条件**: 企業設定情報にて「教室情報表示フラグ」がオンの場合に表示されます。

### 2. 所在地

- **表示形式**: 保存された「都道府県」「市区町村」「以降の住所」を適切にHTMLエスケープして表示します。
- **表示条件**: 企業設定情報にて「教室情報表示フラグ」がオンの場合に教室の所在地を表示します。

### 3. ホームページ

- **表示形式**: 保存された「ホームページ」をテキストリンクとして表示します。
- **動作**: 押下すると別タブで該当のホームページを開きます。
- **表示条件**: 未登録の場合は項目自体を表示しません。
\`\`\`
