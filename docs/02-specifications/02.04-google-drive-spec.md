...
│  または「Google Driveから選択」ボタンでポップアップから選択│
│                                                          │
│  ┌──────────────────────────────────────────────────┐  │
│  │ フォルダID                                        │  │
│  │ folderId                                          │  │
...
│  または「Google Driveから選択」ボタンでフォルダを選択    │
│                                                          │
│  [Google Driveから選択] [ファイル一覧を取得] [インポート]│
│                                                          │
│  ┌──────────────────────────────────────────────────┐  │
│  │ 選択されたファイル (3件)                          │  │
│  │ 📄 ファイル1.pdf                    [削除]        │  │
...
**方法1: Google Picker API（推奨）**
- **ボタン**: 「Google Driveから選択」
- **機能**: Google Driveのファイルピッカーをポップアップで表示
- **特徴**:
  - 視覚的にファイルやフォルダを選択可能
  - **複数選択対応**: Shiftキーを押しながらクリックで複数ファイルを選択可能
  - フォルダ選択も可能
  - ファイルとフォルダの混在選択時は、ファイルのみがインポート対象となる
- **設定要件**:
  - `NEXT_PUBLIC_GOOGLE_API_KEY`: Google APIキー
  - `NEXT_PUBLIC_GOOGLE_CLIENT_ID`: OAuth 2.0クライアントID
  - Google Picker APIの有効化（Google Cloud Console）
...
- Google Picker APIで選択したファイルは、自動的にファイルID入力欄に設定される

#### 3. 操作ボタン

**「Google Driveから選択」ボタン**（推奨）:
- **機能**: Google Picker APIを使用してGoogle Driveからファイルやフォルダを選択
- **表示内容**:
  - Google Driveのファイルピッカーがポップアップで表示
  - ファイルとフォルダを視覚的に選択可能
  - Shiftキーを押しながらクリックで複数選択可能
- **選択後の動作**:
  - 選択したファイル/フォルダのIDが自動的に入力欄に設定される
  - 選択したファイルの一覧が表示される
- **設定要件**: `NEXT_PUBLIC_GOOGLE_API_KEY`と`NEXT_PUBLIC_GOOGLE_CLIENT_ID`が必要
...
- **Google Picker API**: OAuth 2.0を使用してユーザー認証（スコープ: `https://www.googleapis.com/auth/drive.readonly`）

### 10.2 Google Picker API設定
...
**OAuth 2.0スコープ**:
- `https://www.googleapis.com/auth/drive.readonly`: Google Driveファイルの読み取り専用アクセス
