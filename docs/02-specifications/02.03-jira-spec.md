...
2. 課題データを正規化・整形
3. Firestore `jiraIssues`に保存（バッチ処理: 500件/バッチ）
4. ベクトル生成（バッチ処理: 50件/バッチ、10並列、リトライロジック付き）
5. LanceDB `jira_issues`テーブルに保存
6. Firestore `jiraSyncJobs`に同期履歴を記録

### 7.4 進捗ログ

- Firestore保存: 500件ごとに進捗ログ
- ベクトル生成: 50件ごとに進捗ログ
...
2. **インデックス分離**: Lunrインデックスをテーブルごとに管理
