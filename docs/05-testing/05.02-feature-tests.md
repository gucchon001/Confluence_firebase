...
**目的**: Gemini API（Embedding APIとLLM生成API）が正常に動作しているか確認

**テストファイル**: `test-gemini-api-connection.ts`
...
**テスト内容**:
1. **APIキー確認**: `GEMINI_API_KEY`が正しく設定されているか確認
2. **Embedding API疎通**: サンプルテキストで埋め込みベクトル生成の疎通確認
3. **LLM生成API疎通**: Genkit経由で簡単なプロンプトを送信し、テキスト生成の疎通確認

**確認項目**:
- [ ] APIキーが正しく設定されている（長さ20文字以上）
- [ ] Embedding APIが正常に動作する（768次元ベクトルが生成される）
- [ ] LLM生成APIが正常に動作する（Genkit経由でテキストが生成される）
- [ ] 各APIの処理時間が適切（Embedding: 1秒以内、LLM: 3秒以内を目安）

**エラーハンドリング**:
- APIキーが漏洩として報告されている場合（403エラー）: GitHub Secretsの更新を促す
- APIキーが無効な場合（401エラー）: `GEMINI_API_KEY`の値を確認するよう促す
- レート制限に達している場合（429エラー）: しばらく待ってから再試行するよう促す

**注意**: 
- このテストは他の機能テストの前提条件として重要です
- APIキーの問題が疑われる場合は、まずこのテストを実行してください
- テストは`.env.local`から環境変数を読み込みます
...
#### 3.3 ハイブリッド検索と参照元取得テスト

**目的**: ハイブリッド検索が機能し、参照元が正しく取得されているか検証

**テストファイル**: `test-hybrid-search-references.ts`

**実行方法**:
```bash
# 個別実行
npx tsx src/tests/test-hybrid-search-references.ts
```

**テスト内容**:
1. **ハイブリッド検索の動作確認**: ベクトル検索とBM25検索の統合が正しく動作しているか
2. **参照元の検証**: 
   - 参照元のフィールドが完全である（id, title, url, spaceName, labels, distance, source, scoreTextなど）
   - URLが正しく構築されている
   - 重複IDがない
   - スコアが適切である

**確認項目**:
- [ ] ハイブリッド検索が機能している（複数のsourceタイプがある、またはhybrid/compositeスコアがある）
- [ ] 参照元が正しく取得されている（有効参照元率100%）
- [ ] 重複IDがない
- [ ] 参照元のフィールドが完全である
- [ ] URLが正しく構築されている（pageIdがあれば構築可能）
- [ ] 検索結果のスコアが適切（平均スコア >= 0.6で良好）

**注意**: 
- このテストは参照元の品質とハイブリッド検索の機能を統合的に検証します
- URLが'#'でもpageIdがあれば後で構築可能なため、柔軟に判定します
- space_keyがなくてもpageIdがあればURL構築可能です
