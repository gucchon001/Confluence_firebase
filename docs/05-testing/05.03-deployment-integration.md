...
ローカル・本番環境でのデプロイ、型チェック、データ整合性の確認を統合実行するテストです。

## 📚 関連ドキュメント

テストを理解するために、以下のドキュメントを参照してください：

- **[デプロイガイド](../04-operations/04.01-deployment-guide.md)**: デプロイ手順と運用方法
- **[環境変数設定ガイド](../04-operations/04.13-environment-variables.md)**: 環境変数の設定方法
- **[環境別設定ガイド](../04-operations/04.12-environment-setup.md)**: 開発環境のセットアップ方法
- **[拡張スキーマ運用ガイド](../04-operations/04.05-extended-schema-operation-guide.md)**: 拡張スキーマの運用方法
- **[システム全体仕様書](../02-specifications/02.01-spec.md)**: システム全体の機能要件と技術スタック
...
**詳細**: 設定値管理移行テストは`app-config.test.ts`で実行されます
...
```bash
# 設定値の検証（vitestで実行）
npm run test:unit -- src/tests/config/app-config.test.ts
```
...
**テスト結果**:
- 設定値管理移行テストは`app-config.test.ts`で実行されます
- 詳細な結果はテスト実行時に表示されます
