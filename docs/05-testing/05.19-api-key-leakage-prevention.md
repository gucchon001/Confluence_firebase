# APIキー漏洩防止ガイド

## 🚨 重要な注意事項

**APIキー（特にGemini APIキー）は絶対にGitリポジトリにコミットしないでください。**

## 漏洩の可能性がある箇所

### 1. ドキュメントファイル
- ✅ **修正済み**: `docs/04-operations/04.15-jira-production-deployment-guide.md`
  - Gemini APIキーをプレースホルダーに置き換え

### 2. 設定ファイル
- ✅ **修正済み**: `apphosting.yaml`と`setup/apphosting.yaml`
  - `NEXT_PUBLIC_FIREBASE_API_KEY`をプレースホルダーに置き換え
  - 実際の値はFirebase ConsoleまたはSecret Managerで管理

### 3. ログ出力
- ✅ **問題なし**: `src/tests/test-helpers/env-loader.ts`
  - APIキーの最初の10文字のみを出力（完全なキーは出力されない）

## 対応手順

### 1. 新しいGemini APIキーの生成

1. [Google AI Studio](https://makersuite.google.com/app/apikey)にアクセス
2. 新しいAPIキーを生成
3. 古いAPIキーを無効化

### 2. 環境変数の更新

#### ローカル環境
```bash
# .env.local
GEMINI_API_KEY=your-new-gemini-api-key
```

#### GitHub Secrets
```bash
gh secret set GEMINI_API_KEY --body "your-new-gemini-api-key"
```

#### Firebase Secret Manager
```bash
echo -n "your-new-gemini-api-key" | gcloud secrets create gemini_api_key --data-file=-
# または既存のシークレットを更新
echo -n "your-new-gemini-api-key" | gcloud secrets versions add gemini_api_key --data-file=-
```

### 3. Git履歴の確認

既にコミットされている可能性がある場合は、Git履歴を確認：

```bash
# APIキーが含まれているファイルを検索
git log --all --full-history --source -- "**/*.md" "**/*.yaml" | grep -i "AIzaSy"

# 特定のAPIキーが含まれているコミットを検索
git log --all -S "AIzaSyDgbOANBYpo-H5V9-HLwJR36fhNonSz4h4" --source --all
```

### 4. Git履歴からの削除（必要に応じて）

**注意**: この操作は慎重に行ってください。Git履歴を書き換えるため、チームメンバーと調整が必要です。

```bash
# BFG Repo-Cleanerを使用（推奨）
# https://rtyley.github.io/bfg-repo-cleaner/

# または git filter-branch を使用
git filter-branch --force --index-filter \
  "git rm --cached --ignore-unmatch docs/04-operations/04.15-jira-production-deployment-guide.md" \
  --prune-empty --tag-name-filter cat -- --all
```

## apphosting.yamlの設定方法

### 実際のAPIキーの設定

`apphosting.yaml`にはプレースホルダーが記載されています。実際のデプロイ時には、以下のいずれかの方法で実際の値を設定してください：

#### 方法1: Firebase Consoleから設定（推奨）

1. [Firebase Console](https://console.firebase.google.com/)にアクセス
2. プロジェクトを選択
3. App Hosting → Settings → Environment variables
4. `NEXT_PUBLIC_FIREBASE_API_KEY`を追加・編集

#### 方法2: デプロイ時に環境変数を設定

```bash
# デプロイ前に環境変数を設定
export NEXT_PUBLIC_FIREBASE_API_KEY="your-actual-api-key"
firebase deploy --only hosting
```

#### 方法3: ローカルでapphosting.yamlを編集（非推奨）

**注意**: この方法は推奨されません。Gitにコミットしないよう注意してください。

```yaml
# apphosting.yaml（ローカルのみ、Gitにコミットしない）
- variable: NEXT_PUBLIC_FIREBASE_API_KEY
  value: your-actual-api-key  # 実際の値（Gitにコミットしない）
```

## 今後の対策

### 1. プレコミットフックの設定

`.git/hooks/pre-commit`に以下を追加：

```bash
#!/bin/bash
# APIキーのパターンをチェック
if git diff --cached --name-only | xargs grep -E "AIzaSy[A-Za-z0-9_-]{35}" 2>/dev/null; then
  echo "❌ エラー: APIキーが検出されました。コミットを中止します。"
  exit 1
fi
```

### 2. GitHub Actionsでの検証

`.github/workflows/security-check.yml`を作成：

```yaml
name: Security Check

on: [push, pull_request]

jobs:
  check-api-keys:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v3
      - name: Check for API keys
        run: |
          if grep -r "AIzaSy[A-Za-z0-9_-]{35}" --include="*.md" --include="*.yaml" --include="*.ts" --include="*.js" .; then
            echo "❌ エラー: APIキーが検出されました"
            exit 1
          fi
```

### 3. ドキュメントテンプレートの使用

ドキュメントでは常にプレースホルダーを使用：

```markdown
# ❌ 悪い例
GEMINI_API_KEY=AIzaSyDgbOANBYpo-H5V9-HLwJR36fhNonSz4h4

# ✅ 良い例
GEMINI_API_KEY=your-gemini-api-key
```

## 参考リンク

- [Google AI Studio - API Keys](https://makersuite.google.com/app/apikey)
- [GitHub Secrets](https://docs.github.com/en/actions/security-guides/encrypted-secrets)
- [Firebase Secret Manager](https://firebase.google.com/docs/app-hosting/manage-secrets)

