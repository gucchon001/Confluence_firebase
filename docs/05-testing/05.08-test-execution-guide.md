...
- **`keyword-quality-test.ts`**: キーワード品質テスト（統合版）
  - 実行方法: `npx tsx src/tests/keyword-quality-test.ts` （全テストケースを実行）
  - 特定のテストケースのみ実行: `npx tsx src/tests/keyword-quality-test.ts 教室削除問題`
  - 含まれるテストケース: 教室管理、教室削除問題、オファー機能、会員ログイン機能、教室コピー機能

#### ベクトル検索テスト
- **`vector-search-consistency-test.ts`**: ベクトル検索一貫性テスト
  - 実行方法: `npx tsx src/tests/vector-search-consistency-test.ts`
  - 説明: 複数のクエリでベクトル検索の一貫性を検証
