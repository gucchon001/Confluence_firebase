...
**方針**: すべてのテーブルをオンデマンド初期化に統一

**理由**:
1. **メモリ使用量の最小化**: 起動時のメモリ使用量を最小化し、メモリ制限エラーを回避
2. **シンプルな実装**: テーブルごとに異なる初期化戦略を持たない
3. **公平性**: confluenceとjira_issuesのどちらが主要か判断する必要がない

**実装**:
- **confluence**: オンデマンド初期化（検索リクエスト時に初期化）
...
| 初期化方法 | 起動時のメモリ使用量 | 備考 |
|-----------|-------------------|------|
| **両方のテーブルを起動時に初期化** | 約24GB | ❌ メモリ制限エラー |
| **主要テーブルのみ起動時に初期化** | 約12GB | ⚠️ まだメモリ使用量が多い |
| **両方ともオンデマンド初期化** | 約0GB（起動時） | ✅ **推奨（実装済み）** - 初回検索が遅いが、メモリ使用量を最小化 |

### パフォーマンスへの影響

#### オンデマンド初期化（すべてのテーブル）✅ **実装済み**
...
- **初回検索**: 初期化時間がかかる（最大180秒）
  - confluence検索: 初回のみ遅い
  - jira_issues検索: 初回のみ遅い
- **2回目以降の検索**: 高速（既に初期化済み）

**推奨される運用**:
- 初回検索の遅延を許容できる場合: オンデマンド初期化 ✅ **現在の実装**
- 初回検索を高速化したい場合: 環境変数で起動時に初期化するテーブルを指定（将来の拡張案）
