...
**影響**:
- 2つのインスタンスが常時起動している
- 各インスタンスがLanceDBのメモリマップドファイルを読み込む
- **メモリ使用量が実質的に2倍になる可能性**

**計算**:
- 1インスタンスあたりのメモリ使用量: 約4GB（external + arrayBuffers = 12GB、ただしメモリマップドファイルは共有される可能性あり）
- 2インスタンス: 約8GB（メモリマップドファイルが共有されない場合）
...
1. **`minInstances: 2`への増加**（11月18日）
   - 2つのインスタンスが常時起動
   - 各インスタンスがLanceDBのメモリマップドファイルを読み込む
   - メモリ使用量が実質的に2倍になる

2. **LanceDBのメモリマップドファイル**
...
**推奨される対策**:
- `minInstances: 1`に戻す（メモリ使用量を削減）
- または、LanceDBのメモリマップドファイルの使用を最小化する（バッチ処理の実装済み）
