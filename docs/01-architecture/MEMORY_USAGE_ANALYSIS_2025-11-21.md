...
#### 3. **バランスの取れた初期化戦略** ✅ **実装済み**

**方針**: メモリ使用量とパフォーマンスのバランスを取る

**理由**:
1. **初回検索の高速化**: confluenceは起動時に非同期初期化することで、初回検索でもBM25検索が利用可能
2. **メモリ使用量の最適化**: jira_issuesはオンデマンド初期化で、必要時のみメモリに読み込む
3. **非ブロッキング**: 起動時の初期化は非同期で実行されるため、アプリケーションの起動をブロックしない

**実装**:
- **confluence**: 起動時に非同期で初期化（バックグラウンド、非ブロッキング）
  - 初回検索でもBM25検索が利用可能（初期化が完了していれば）
  - 初期化が完了していない場合は、ベクトル検索のみで即座に返す
- **jira_issues**: オンデマンド初期化（検索リクエスト時に初期化）
  - メモリ使用量を最小化（必要時のみ読み込む）

**パフォーマンスへの影響**:
- **起動時**: 非ブロッキングでconfluenceを初期化（アプリケーション起動は即座に完了）
- **初回検索**: 
  - 初期化が完了していれば: BM25検索も利用可能（高速）
  - 初期化が完了していなければ: ベクトル検索のみで即座に返す（BM25はスキップ）
- **2回目以降の検索**: 高速（既に初期化済み）
- **メモリ使用量**: 約12GB（confluenceのみ起動時に初期化）
...
| 初期化方法 | 起動時のメモリ使用量 | 初回検索のパフォーマンス | 備考 |
|-----------|-------------------|----------------------|------|
| **両方のテーブルを起動時に初期化** | 約24GB | 高速（BM25利用可能） | ❌ メモリ制限エラー |
| **confluenceのみ非同期初期化、jira_issuesはオンデマンド** | 約12GB | 高速（初期化完了時）またはベクトル検索のみ | ✅ **推奨（実装済み）** - バランスが良い |
| **両方ともオンデマンド初期化** | 約0GB（起動時） | 遅い（初期化待ち最大180秒） | ❌ 運用不可（初回検索が3分かかる） |

### パフォーマンスへの影響

#### バランスの取れた初期化戦略 ✅ **実装済み**

**confluence（起動時に非同期初期化）**:
- **起動時**: 非ブロッキングでバックグラウンド初期化
- **メモリ使用量**: 約12GB（初期化完了後）
- **初回検索**: 
  - 初期化が完了していれば: BM25検索も利用可能（高速）
  - 初期化が完了していなければ: ベクトル検索のみで即座に返す（BM25はスキップ）
...
**jira_issues（オンデマンド初期化）**:
- **起動時**: 初期化しない（メモリ使用量0GB）
- **初回検索**: 初期化時間がかかる（最大180秒）
  - 初期化が完了するまで: ベクトル検索のみで即座に返す（BM25はスキップ）
  - 初期化完了後: BM25検索も利用可能
- **2回目以降の検索**: 高速（既に初期化済み）
- **メモリ使用量**: 必要時のみ増加（約12GB）

**推奨される運用**:
- ✅ **現在の実装**: confluenceは起動時に非同期初期化、jira_issuesはオンデマンド初期化
- メモリ使用量とパフォーマンスのバランスが取れている
- 初回検索も即座に返す（初期化が完了していなくてもベクトル検索のみで動作）
