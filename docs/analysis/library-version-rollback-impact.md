...
> **2025-11-09 追記**  
> BOM エラーの最終原因は Gemini API キーに混入した BOM だったため、REST API 直接呼び出しとキーのサニタイズで解決済みです。  
> したがってライブラリをロールバックする必要はなく、本ドキュメントは過去検討の記録として残しています。

## 現在の状況

### 現在のライブラリとモデル
- **ライブラリ**: `@google/generative-ai@0.23.0`（LLM用／EmbeddingはREST API直接呼び出し）
...
- しかし、**最終的な根本原因は`@google/generative-ai`ライブラリではなく、Secret Manager 由来の API キーに混入した BOM**
- APIキーサニタイズと REST API 直接呼び出しの現行実装で解決済み
...
1. **現行実装（APIキーサニタイズ + REST呼び出し）を維持しつつ監視**
   - デプロイ後のログで ByteString エラーが発生しないことを確認（済）
   - Secret を更新する際は BOM が含まれない UTF-8 で登録する

2. **追加対策が必要な場合**
   - `@google/generative-ai` の LLM 用 SDK を最新版に追随しつつ、Genkit 初期化時にサニタイズを継続
