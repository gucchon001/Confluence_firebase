...
- 各チャンクの埋め込みベクトルを生成（Xenova Transformersライブラリ使用、384次元、L2正規化）
...
### 3. ハイブリッド検索と回答生成
- ユーザーの質問をベクトル化（Xenova Transformersライブラリ使用、384次元、L2正規化）
- キーワード抽出と前処理（高優先度・低優先度キーワードの分類）
