...
#### 01. デプロイガイド
**ファイル**: [`04.01-deployment-guide.md`](./04.01-deployment-guide.md)  
**目的**: 本番環境へのデプロイ手順と設定  
...
#### 02. GitHub Actions設定
**ファイル**: [`04.02-github-actions-setup.md`](./04.02-github-actions-setup.md)  
**目的**: CI/CDパイプラインの設定  
**内容**: GitHub Actionsの設定方法、ワークフロー定義  
**重要度**: ⭐⭐

#### 03. データ同期戦略
**ファイル**: [`04.03-data-synchronization-strategy.md`](./04.03-data-synchronization-strategy.md)  
**目的**: データソースの更新頻度と管理戦略  
...
**⚠️ 重要**: データ同期、インデックス作成、GCSアップロードを行う場合は、**必ず完全再構築スクリプトを使用してください**。詳細は[完全再構築スクリプト運用ガイド](./04.19-full-rebuild-scripts-guide.md)を参照してください。

#### 04. バックアップ管理
**ファイル**: [`04.04-backup-management-guide.md`](./04.04-backup-management-guide.md)  
**目的**: データバックアップと復元  
**内容**: 自動バックアップ（毎日午前2時）、手動バックアップ・復元手順  
**重要度**: ⭐⭐

#### 05. 拡張スキーマ運用ガイド
**ファイル**: [`04.05-extended-schema-operation-guide.md`](./04.05-extended-schema-operation-guide.md)  
**目的**: LanceDB拡張スキーマの運用  
**内容**: LanceDB拡張スキーマ（StructuredLabel統合版）の運用、日常的な同期処理、インデックス管理  
**重要度**: ⭐⭐⭐

#### 06. スクリプト利用ガイド
**ファイル**: [`04.06-scripts-guide.md`](./04.06-scripts-guide.md)  
**目的**: 運用スクリプトの使用方法  
**内容**: 全スクリプトの用途・実行方法  
**重要度**: ⭐⭐⭐

#### 07. リポジトリ移管ガイド
**ファイル**: [`04.07-migration-guide.md`](./04.07-migration-guide.md)  
**目的**: リポジトリ移管時の手順  
**内容**: 必要ファイルのチェックリスト  
**重要度**: ⭐

#### 08. ネットワーク共有ガイド
**ファイル**: [`04.08-network-sharing-guide.md`](./04.08-network-sharing-guide.md)  
**目的**: ローカル開発環境の共有方法  
**内容**: ネットワーク共有設定、アクセス方法  
**重要度**: ⭐

#### 12. 環境別設定ガイド
**ファイル**: [`04.12-environment-setup.md`](./04.12-environment-setup.md)  
**目的**: ローカル環境と本番環境での設定方法  
**内容**: 環境別の設定ファイル、設定手順  
**重要度**: ⭐⭐⭐

#### 13. 環境変数設定ガイド
**ファイル**: [`04.13-environment-variables.md`](./04.13-environment-variables.md)  
**目的**: 環境変数の一元管理と設定方法  
**内容**: 必須環境変数一覧、検証ルール、設定方法  
**重要度**: ⭐⭐⭐

#### 14. クイックスタートガイド
**ファイル**: [`04.14-quick-start.md`](./04.14-quick-start.md)  
**目的**: 他PCでLanceDB同期を実行する手順  
**内容**: 必要なファイルの準備、依存関係のインストール、同期の実行、トラブルシューティング  
**重要度**: ⭐⭐

#### 15. Jira本番環境デプロイガイド
**ファイル**: [`04.15-jira-production-deployment-guide.md`](./04.15-jira-production-deployment-guide.md)  
**目的**: Jiraデータの本番環境へのデプロイ手順  
**内容**: Jiraデータ同期、インデックス作成、GCSアップロード、デプロイフロー  
**重要度**: ⭐⭐⭐

#### 16. GitHub CLIとActionsガイド
**ファイル**: [`04.16-github-cli-and-actions-guide.md`](./04.16-github-cli-and-actions-guide.md)  
**目的**: GitHub CLIとActionsの使用方法  
**内容**: GitHub CLIのインストール、認証、Actionsの設定  
**重要度**: ⭐⭐

#### 17. Phase1-Phase2デプロイガイド
**ファイル**: [`04.17-phase1-phase2-deployment-guide.md`](./04.17-phase1-phase2-deployment-guide.md)  
**目的**: Phase1とPhase2のデプロイ手順  
**内容**: Phase1とPhase2のデプロイフロー、注意事項  
**重要度**: ⭐⭐

#### 18. LunrキャッシュGCS運用ガイド
**ファイル**: [`04.18-lunr-cache-gcs-operation-guide.md`](./04.18-lunr-cache-gcs-operation-guide.md)  
...
#### 19. 完全再構築スクリプト運用ガイド
**ファイル**: [`04.19-full-rebuild-scripts-guide.md`](./04.19-full-rebuild-scripts-guide.md)  
...
#### 20. ドキュメント整理・統合サマリー
**ファイル**: [`04.20-document-consolidation-summary.md`](./04.20-document-consolidation-summary.md)  
**目的**: ドキュメント整理・統合の記録  
**内容**: 完全再構築スクリプトの推奨化、古い情報を含むドキュメントのアーカイブ  
**重要度**: ⭐

#### 21. 古いスクリプト・コマンド整理サマリー
**ファイル**: [`04.21-old-scripts-cleanup-summary.md`](./04.21-old-scripts-cleanup-summary.md)  
**目的**: 完全再構築スクリプトと競合する可能性のある古いスクリプト・コマンドの整理  
**内容**: 非推奨コマンド一覧、推奨代替方法、スクリプトの関係性  
**重要度**: ⭐⭐  
**⚠️ 注意**: 古いコマンド（`sync:and:upload`、`sync:full:and:upload`、`deploy:data`）は使用を避けてください。  
**目的**: LunrキャッシュのGCS統合機能の運用  
**内容**: 動作確認、手動操作、トラブルシューティング、メンテナンス  
**重要度**: ⭐⭐⭐  
**関連**: [アーキテクチャ設計](../01-architecture/01.05.01-lunr-cache-gcs-integration.md)
...
3. **データ同期を設定する**
   - [`04.19-full-rebuild-scripts-guide.md`](./04.19-full-rebuild-scripts-guide.md) - **完全再構築スクリプト（推奨）**
   - [`04.03-data-synchronization-strategy.md`](./04.03-data-synchronization-strategy.md) - データ同期戦略
...
│
├── 【メインドキュメント】運用ガイド
│   ├── 04.01-deployment-guide.md               # デプロイガイド（統合版）
│   ├── 04.02-github-actions-setup.md           # GitHub Actions設定
│   ├── 04.03-data-synchronization-strategy.md  # データ同期戦略
│   ├── 04.04-backup-management-guide.md        # バックアップ管理
│   ├── 04.05-extended-schema-operation-guide.md # 拡張スキーマ運用ガイド
│   ├── 04.06-scripts-guide.md                  # スクリプト利用ガイド
│   ├── 04.07-migration-guide.md                # リポジトリ移管ガイド
│   ├── 04.08-network-sharing-guide.md          # ネットワーク共有ガイド
│   ├── 04.12-environment-setup.md              # 環境別設定ガイド
│   ├── 04.13-environment-variables.md          # 環境変数設定ガイド
│   ├── 04.14-quick-start.md                    # クイックスタートガイド
│   ├── 04.15-jira-production-deployment-guide.md # Jira本番環境デプロイガイド
│   ├── 04.16-github-cli-and-actions-guide.md   # GitHub CLIとActionsガイド
│   ├── 04.17-phase1-phase2-deployment-guide.md # Phase1-Phase2デプロイガイド
│   ├── 04.18-lunr-cache-gcs-operation-guide.md # LunrキャッシュGCS運用ガイド
│   └── 04.19-full-rebuild-scripts-guide.md     # 完全再構築スクリプト運用ガイド（推奨）
│
└── 【履歴ドキュメント】過去のデプロイ記録
    ├── 04.09-emergency-deployment-20251020.md   # 緊急デプロイ手順 (2025-10-20)
    ├── 04.10-content-truncation-improvement-deployment.md # コンテンツ切り詰め改善適用ガイド
    ├── 04.11-postbuild-file-copy-solution.md    # ビルド後ファイルコピーソリューション
    └── 04.20-document-consolidation-summary.md  # ドキュメント整理・統合サマリー
```
