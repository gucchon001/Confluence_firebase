...
#### `sync-confluence-full-rebuild.ts` / `sync-jira-full-rebuild.ts` - 完全再構築スクリプト

**用途**: データ同期、インデックス作成、GCSアップロードを一括で実行します。
...
- **このスクリプトはLanceDBテーブルの再構築のみを行います**（インデックス作成やGCSアップロードは含まれません）

**⚠️ 完全再構築スクリプトとの違い**:
- `rebuild-lancedb-smart-chunking.ts`: LanceDBテーブルの再構築のみ（チャンキング最適化）
- `sync-confluence-full-rebuild.ts` / `sync-jira-full-rebuild.ts`: データ同期→インデックス作成→GCSアップロードの完全なパイプライン

**推奨**: 通常の運用では、**完全再構築スクリプト（`sync-confluence-full-rebuild.ts` / `sync-jira-full-rebuild.ts`）を使用してください**。詳細は[完全再構築スクリプト運用ガイド](./04.19-full-rebuild-scripts-guide.md)を参照してください。

**関連ドキュメント**:
- [拡張スキーマ運用ガイド](./04.05-extended-schema-operation-guide.md)
- [完全再構築スクリプト運用ガイド](./04.19-full-rebuild-scripts-guide.md)
