...
#### 1. データ整合性テスト（Jiraテーブル未対応）

**`scripts/test-data-integrity.ts`**:
- **現状**: `confluence`テーブルのみを対象
- **問題**: Jiraテーブル（`jira_issues`）の検証が含まれていない
- **影響**: `deploy:data`スクリプト実行時、Jiraテーブルの整合性が検証されない

#### 2. アップロード/ダウンロードスクリプトの直接テスト
...
#### 1. `test-data-integrity.ts`の拡張（優先度: 高）

**必要な機能**:
- Jiraテーブル（`jira_issues`）の検証を追加
- テーブルごとの検証結果を分離
- Jiraテーブル用のサンプルIssue Key検証

**実装方針**:
- `--table`オプションを追加（`confluence` | `jira_issues` | `all`）
- Jiraテーブル用の検証ロジックを追加
- Issue Keyベースの検証（`page_id`の代わりに`issue_key`を使用）

#### 2. アップロード/ダウンロードスクリプトのテスト追加（優先度: 中）
