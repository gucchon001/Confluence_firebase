...
- **`src/lib/jira-sync-service.ts`**: 同期処理の実装
  - ✅ **差分取得機能**: 既存データの`updated`フィールドと比較して、変更されたデータのみ更新
  - ベクトル生成（バッチ処理、並列化、リトライ機能付き）
  - Firestore保存（バッチ処理: 50件/バッチ、変更があったもののみ）
- LanceDBテーブル更新（`jira_issues`、差分アップサート／テーブルが空の場合は全件再構築）
  - 統計情報: `added`（追加）、`updated`（更新）、`unchanged`（変更なし）の件数を記録
...
> **⚠️ 重要**: データ同期、インデックス作成、GCSアップロードを行う場合は、**必ず完全再構築スクリプトを使用してください**。個別コマンドを手動で実行すると、処理順序の誤りや漏れが発生する可能性があります。
>
> 詳細は [完全再構築スクリプト運用ガイド](./04.19-full-rebuild-scripts-guide.md) を参照してください。
...
<details>
<summary>個別コマンド実行（処理順序の誤りや漏れが発生する可能性があるため非推奨）</summary>
...
- **実装済み**: ✅ **差分取得機能**: 変更されたデータのみ更新（既存データの`updated`フィールドと比較）
- **実装済み**: Jira同期 → Lunrインデックス初期化 → LanceDBインデックス作成 → GCSアップロードの完全なパイプライン
- **実装済み**: エラーハンドリングと詳細なログ出力（追加・更新・変更なしの統計情報を含む）
...
**解決方法**:
1. ローカル環境でJiraデータを完全再構築: `npm run sync:jira:rebuild`
2. ビルド時にダウンロードされることを確認

> **注意**: 完全再構築スクリプトを使用すれば、データ同期からインデックス作成、GCSアップロードまで自動的に実行されます。
...
**解決方法**:
1. ローカル環境でJiraデータを完全再構築: `npm run sync:jira:rebuild`
2. キャッシュパスを確認（Cloud Run環境では`.next/standalone/.cache`を使用）

> **注意**: 完全再構築スクリプトを使用すれば、Lunrインデックス初期化からGCSアップロードまで自動的に実行されます。
...
> **注意**: 完全再構築スクリプトを使用すれば、LanceDBインデックス作成からGCSアップロードまで自動的に実行されます。
...
| `GEMINI_API_KEY` | Gemini APIキー | `GEMINI_API_KEY=your-gemini-api-key` | 同上 |
...
# 2. PowerShellのパスをリフレッシュ（インストール直後の場合）
# 方法A: PowerShellを再起動する（推奨）
# 方法B: パスをリフレッシュするスクリプトを実行
...
- [GitHub CLIとGitHub Actions運用ガイド](./04.16-github-cli-and-actions-guide.md) - GitHub CLIの使用方法とGitHub Actionsワークフローの運用
