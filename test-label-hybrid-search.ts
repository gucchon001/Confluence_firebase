...
async function testLabelHybridSearch() {
  log('🔍 ラベル機能とハイブリッド検索のテストを開始...\n');
  ...
  try {
    ...
    try {
      // 除外ラベルを含む検索（結果が少なくなるはず）
      const excludedResults = await searchLanceDB({
        query: '管理機能',
        topK: 10,
        filters: {},
      ...
      log(`📊 除外なし検索結果: ${excludedResults.length}件`);
      ...
      log(`📊 除外効果: ${excludedResults.length - filteredResults.length}件が除外されました`);
...
testLabelHybridSearch().catch(console.error);
