/**
 * Confluence APIを通して「アーカイブ」ラベルがついているページの一覧を取得
 */
...
async function getArchivePages(): Promise<ArchivePagesResult> {
  try {
    console.log('🔍 「アーカイブ」ラベルがついているページを検索中...');
    ...
    // CQLクエリで「アーカイブ」ラベルを検索
    const cqlQuery = spaceKey 
      ? `space = "${spaceKey}" AND label = "アーカイブ"`
      : `label = "アーカイブ"`;
    ...
    if (!response.data.results || response.data.results.length === 0) {
      console.log('⚠️ 「アーカイブ」ラベルがついているページが見つかりませんでした。');
      return {
        totalPages: 0,
        pages: [],
    ...
    const result: ArchivePagesResult = {
    ...
    // 結果をJSONファイルに保存
    const filename = `archive-pages-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
    const fs = require('fs');
...
async function main() {
  try {
    const result = await getArchivePages();
    console.log(`\n✅ 完了: ${result.totalPages}件の「アーカイブ」ラベル付きページを取得しました`);
  } catch (error) {
